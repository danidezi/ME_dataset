...
\myHighlight{${\cal S}, {\cal S}_1, {\cal S}_2.$}\coordHE{} We illustrate this situation by following fundamental example.
...
both slits are open; only \myHighlight{$j$}\coordHE{}th slit is open, \myHighlight{$j=1,2.$}\coordHE{} The main distinguishing feature
...
\myHighlight{${\cal S}=$}\coordHE{}{both slits are open}, \myHighlight{${\cal S}_j=$}\coordHE{}{only jth slit is open}, \myHighlight{$j=1,2.$}\coordHE{} 

In fact, probabilities in (\ref{F2}) are related to these three contexts.
Thus \myHighlight{$P=P_{{\cal S}}(A)$}\coordHE{} and \myHighlight{$P_j= P_{{\cal S}_j/{\cal S}} P_{{\cal S}_j}(A), j=1,2.$}\coordHE{} 

Here we use various context-indexes. The \myHighlight{$P_{{\cal S}}(A), P_{{\cal S}_j}(A)$}\coordHE{} denote probabilities
of an event \myHighlight{$A$}\coordHE{} with respect to various contexts. The coefficients 
\myHighlight{$P_{{\cal S}_j/{\cal S}}, j=1,2,$}\coordHE{} have another
meaning. In general these
are not probabilities of \myHighlight{${\cal S}_j$}\coordHE{} with respect to the context \myHighlight{${\cal S}$}\coordHE{}
(besides some very special, "classical", situations), because the context
\myHighlight{${\cal S}_j$}\coordHE{} in general is not an event for the context \myHighlight{${\cal S}.$}\coordHE{}
The \myHighlight{$P_{{\cal S}_j/{\cal S}}, j=1,2,$}\coordHE{}
are kinds of {\it balance probabilities}. These are proportion coefficients in splitting
of the ensemble \myHighlight{$S$}\coordHE{} prepared on the basis of the complex of physical conditions \myHighlight{${\cal S}$}\coordHE{}
into ensembles \myHighlight{$S_j$}\coordHE{} prepared on the basis of the complexes of physical conditions \myHighlight{${\cal S}_j:$}\coordHE{}
\begin{equation}\coord{}\boxEquation{
\label{BP}
...
where \myHighlight{$N$}\coordHE{} is the number of elements in \myHighlight{$S$}\coordHE{} and \myHighlight{$N_j$}\coordHE{} 
is the number of elements in \myHighlight{$S_j,$}\coordHE{} see [22], [23] for the details.
...
two preparation procedures: \myHighlight{$j$}\coordHE{}th slit is open, \myHighlight{$j=1,2.$}\coordHE{}

However, the balance probabilities \myHighlight{$P_{{\cal S}_j/{\cal S}}$}\coordHE{}  would not play 
so important role in our considerations. The crucial role will be played by contextual
probabilities \myHighlight{$P_{{\cal S}}(A), P_{{\cal S}_j}(A).$}\coordHE{} 
...
if \myHighlight{${\cal S}_1\cap {\cal S}_2 = \emptyset$}\coordHE{} and \myHighlight{${\cal S}_1\cup {\cal S}_2 ={\cal S}.$}\coordHE{}
...
set \myHighlight{$P=P(A)$}\coordHE{} and \myHighlight{$P_j= P({\cal S}_1) P(A/{\cal S}_1).$}\coordHE{}
...
The main physical assumption is that this formula is  derived for one fixed context \myHighlight{${\cal S}$}\coordHE{}
...
It is also important that contexts \myHighlight{${\cal S}_1,  {\cal S}_2$}\coordHE{} 
can be realized as elements of the field of events corresponding
to the context \myHighlight{${\cal S}.$}\coordHE{}
...
\myHighlight{$P_{{\cal S}_j/{\cal S}}$}\coordHE{} can be interpreted as 
\myHighlight{$P_{{\cal S}}$}\coordHE{}-probabilities, \myHighlight{$P_{{\cal S}_j/{\cal S}}=P_{{\cal S}} ({\cal S}_j)$}\coordHE{} 
and contextual probabilities \myHighlight{$P_{{\cal S}_j}(A)$}\coordHE{}
as conditional probabilities with respect to the context \myHighlight{${\cal S}, P_{{\cal S}_j}(A)=
P_{{\cal S}}(A/{\cal S}_j).$}\coordHE{}  Here the contextual 
probabilities are given by Bayes' formula
(the additional postulate of Kolmogorov's probability theory):

\myHighlight{$P_{{\cal S}_j}(A)= P_{{\cal S}}(A \cap {\cal S}_j)/ P_{{\cal S}}({\cal S}_j) .$}\coordHE{}

In general, there are no reasons to assume that new complexes of conditions \myHighlight{${\cal S}_j$}\coordHE{}
...
preparation/measurement procedures. The context \myHighlight{${\cal S}$}\coordHE{} produced an ensemble of 
particles \myHighlight{$S$}\coordHE{} that passed through the screen with slits  when both slits are open. A measurement 
procedure is the measurement of the position of a particle on the registration screen.
In the same way the context \myHighlight{${\cal S}_j$}\coordHE{} produced an ensemble of 
particles \myHighlight{$S_j$}\coordHE{} that passed through the screen with slits when only \myHighlight{$j$}\coordHE{}th slit is open.}
...
paper [27] he even used the symbol \myHighlight{$P(A|{\cal S}),$}\coordHE{} where \myHighlight{${\cal S}$}\coordHE{} is a complex of conditions,
- an analogue of our symbol \myHighlight{$P_{{\cal S}}(A).$}\coordHE{}}
...
Let \myHighlight{${\cal S}$}\coordHE{} and \myHighlight{${\cal S}_j, j=1,2,$}\coordHE{} be three
...
\myHighlight{$P_{{\cal S}_j/{\cal S}}.$}\coordHE{}  These are proportional coefficients for numbers of physical systems
obtained after preparations under the complexes of physical conditions \myHighlight{${\cal S}$}\coordHE{} and \myHighlight{${\cal S}_j.$}\coordHE{}
If (starting with the same number of particles)  we get \myHighlight{$N$}\coordHE{} and \myHighlight{$N_j$}\coordHE{} systems after  \myHighlight{${\cal S}$}\coordHE{}
and \myHighlight{${\cal S}_j$}\coordHE{} preparations, respectively, then \myHighlight{$P_{{\cal S}_j/{\cal S}}$}\coordHE{}  are defined by
(\ref{BP}). We assume that balance probabilities satisfy to balance equation (\ref{BC}).
This is quite natural conditions: splitting (\ref{CT}) of the context \myHighlight{${\cal S}$}\coordHE{} 
...
\myHighlight{$\delta(A;{\cal S}; {\cal S}_j)=P_{{\cal S}_1/{\cal S}} [P_{{\cal S}}(A)-
P_{{\cal S}_1}(A)]+ P_{{\cal S}_2/{\cal S}} [P_{{\cal S}}(A)-
P_{{\cal S}_2}(A)].$}\coordHE{}

This quantity describes the deformation of probability distribution \myHighlight{$P_{{\cal S}}$}\coordHE{}
due to context transitions.
\medskip

By using balance equation (\ref{BC}) we get:

\myHighlight{$P_{{\cal S}}(A)= P_{{\cal S}}(A) P_{{\cal S}_1/{\cal S}}  + P_{{\cal S}}(A)P_{{\cal S}_2/{\cal S}} .$}\coordHE{}
...
stable transformations: If \myHighlight{${\cal S}_j \rightarrow {\cal S}, j=1,2,$}\coordHE{} 
i.e., \myHighlight{$\delta(A; {\cal S}, {\cal S}_j) \rightarrow 0$}\coordHE{}, 
...
The perturbation term \myHighlight{$\delta(A; {\cal S}, {\cal S}_j)$}\coordHE{} depends on absolute magnitudes of probabilities. 
It would be natural to introduce normalized coefficient of the context transition 

\myHighlight{$\lambda(A; {\cal S}, {\cal S}_j)=
\frac{\delta(A; {\cal S}, {\cal S}_j)}{2\sqrt{P_{{\cal S}_1/{\cal S}}  P_{{\cal S}_1}(A)P_{{\cal S}_2/{\cal S}}  P_{{\cal S}_2}(A)}},$}\coordHE{}

that gives the relative measure of statistical deviations due to the transition from one complex of conditions, 
\myHighlight{${\cal S},$}\coordHE{} to others, \myHighlight{${\cal S}_j.$}\coordHE{} Transformation (\ref{F3}) can be written in the form:
...
1). \myHighlight{$|\lambda(A; {\cal S}, {\cal S}_j)|\leq 1;$}\coordHE{}

2). \myHighlight{$|\lambda(A; {\cal S}, {\cal S}_j)|\geq 1.$}\coordHE{}

In both cases it is convenient to introduce a new context transition parameter 
\myHighlight{$\theta= \theta(A; {\cal S}, {\cal S}_j)$}\coordHE{} and represent
...
Here \myHighlight{$c$}\coordHE{} is the diagonal of the parallelogram with sides \myHighlight{$a$}\coordHE{} and \myHighlight{$b$}\coordHE{} and the angle \myHighlight{$\theta$}\coordHE{}
between these sides. Of course, the parallelogram law is  just the law of linear
(two dimensional Hilbert space) algebra: for finding the length \myHighlight{$c$}\coordHE{} of the sum \myHighlight{${\bf c}$}\coordHE{}
of vectors \myHighlight{${\bf a}$}\coordHE{} and \myHighlight{${\bf b}$}\coordHE{} having lengths \myHighlight{$a$}\coordHE{} and \myHighlight{$b$}\coordHE{} and the angle \myHighlight{$\theta$}\coordHE{} between them.
...
\myHighlight{${\cal S}\rightarrow {\cal S}_j$}\coordHE{} can be described by the wave:
$$\coord{}\boxMath{
...
\myHighlight{$P_{{\cal S}_j/{\cal S}} =1/2, j=1,2.$}\coordHE{} By the symbol \myHighlight{$A_x, x \in {\bf R},$}\coordHE{}
is denoted the event of  the registration of a particle at the point \myHighlight{$x$}\coordHE{} 
ofd the straight line. We set:

\myHighlight{$ p(x)= P_{{\cal S}}(A_x)$}\coordHE{} and \myHighlight{$p_j(x)= P_{{\cal S}_j}(A_x), j=1,2,$}\coordHE{}

where contexts \myHighlight{${\cal S}$}\coordHE{} and \myHighlight{${\cal S}_j$}\coordHE{} were defined in Example 1.
By using
(\ref{F5}) we get:

\myHighlight{$p(x)= \frac{1}{2} [p_1(x)+p_2(x)+ 2 \sqrt{p_1(x)p_2(x)} \cos \theta(x)] .$}\coordHE{}

By using (\ref{TTT}) we represent this probability as the square of a complex
amplitude, \myHighlight{$p(x)= \vert \phi(x)\vert^2,$}\coordHE{}
...
and phases \myHighlight{$\theta_j(x)$}\coordHE{} are chosen in such a way that the phase shift
\myHighlight{$\theta_1(x) - \theta_2(x) = \theta (x).$}\coordHE{} We also introduce  complex amplitudes
for probabilities \myHighlight{$p_j(x): \; \phi_j(x)= \frac{1}{\sqrt{2}} e^{i \theta_j(x)}\sqrt{p_j(x)}.$}\coordHE{}
Here \myHighlight{$p_j(x)= \vert \phi_j(x)\vert^2.$}\coordHE{} The complex amplitudes are 
said to be {\it wave functions:}  \myHighlight{$\phi(x)$}\coordHE{} is the wave function on (the straight
line of) the registration screen for both slits are open;
\myHighlight{$\phi_j(x)$}\coordHE{} is the wave function on (the straight
line of) the registration screen for \myHighlight{$j$}\coordHE{}th slit  is open.

Let us set \myHighlight{$\xi(x)=\frac{\theta(x)}{h},$}\coordHE{} where \myHighlight{$h>0$}\coordHE{} is some scaling factor.
We have:
$$\coord{}\boxMath{
...
By choosing \myHighlight{$h$}\coordHE{} as the Planck constant we get a quantum-like representation
...
Suppose now that \myHighlight{$\xi$}\coordHE{} depends linearly on \myHighlight{$x: \xi_j(x)= \frac{{\bf p}_j x}{h}, \xi(x)= \frac{{\bf p} x}{h}, 
{\bf p} ={\bf p}_1 - {\bf p}_2.$}\coordHE{} Under such an assumption we shall get interference
of two `free-waves' corresponding to momentums \myHighlight{${\bf p}_1$}\coordHE{} and \myHighlight{${\bf p}_2.$}\coordHE{} Of course, this
...
Each event \myHighlight{$A$}\coordHE{} generates the dichotomic variable \myHighlight{$a:$}\coordHE{}  \myHighlight{$a=a_1$}\coordHE{} if \myHighlight{$A$}\coordHE{} occurs and \myHighlight{$a=a_2$}\coordHE{} if \myHighlight{$A$}\coordHE{} does not
occur. Values \myHighlight{$a_1$}\coordHE{} and \myHighlight{$a_2$}\coordHE{} do not play any role in our considerations; in principle,
we can consider the case \myHighlight{$a_1=0$}\coordHE{} and \myHighlight{$a_2=1.$}\coordHE{}

{\bf Definition.} {\it A physical observable \myHighlight{$a$}\coordHE{} is incompatible with a pair \myHighlight{${\cal S}_1, {\cal S}_2$}\coordHE{} of contexts 
if there exists  a context \myHighlight{${\cal S}$}\coordHE{}  such
that \myHighlight{$\delta(a=a_i; {\cal S}; {\cal S}_j)\not=0.$}\coordHE{}}

In such a case a transition from the complex of physical conditions \myHighlight{${\cal S}$}\coordHE{} to complexes
\myHighlight{${\cal S}_j$}\coordHE{} induces non-negligible statistical deviations for \myHighlight{$a$}\coordHE{}-measurements. It is not the same
to measure \myHighlight{$a$}\coordHE{} under the complex of conditions \myHighlight{${\cal S}$}\coordHE{} or \myHighlight{${\cal S}_j.$}\coordHE{}\footnote{
We need to consider two complexes \myHighlight{${\cal S}_1$}\coordHE{} and \myHighlight{${\cal S}_2,$}\coordHE{} because we would like to 
consider another dichotomic variable \myHighlight{$b$}\coordHE{} connected to these contexts.}
...
Let \myHighlight{${\cal H}$}\coordHE{} be the two dimensional Hilbert space. Rays of this space represent 
some class of complexes of physical conditions. 
Let the dichotomic variable \myHighlight{$a$}\coordHE{} be represented  by a self-adjoint operator (symmetric
matrix) \myHighlight{$\hat{a}.$}\coordHE{} We remark that we can associate with any physical observable \myHighlight{$a$}\coordHE{} 
two complexes of conditions \myHighlight{${\cal S}_1^a, {\cal S}_2^a$}\coordHE{} 
namely contexts corresponding to eigenvectors \myHighlight{$\phi_1^a, \phi_2^a$}\coordHE{} of  \myHighlight{$\hat{a}.$}\coordHE{} 
The \myHighlight{${\cal S}_j^a$}\coordHE{} describes the filter with respect to the value \myHighlight{$a=a_j$}\coordHE{}.

Let us consider other dichotomic physical observable \myHighlight{$b=b_1, b_2.$}\coordHE{} It is represented
by a self-adjoint operator \myHighlight{$\hat{b}$}\coordHE{} with eigenvectors \myHighlight{$\phi_1^b, \phi_2^b.$}\coordHE{} 
These eigenvectors represent contexts \myHighlight{${\cal S}_1^b, {\cal S}_2^b$}\coordHE{} 
(filtrations corresponding to \myHighlight{$b=b_1$}\coordHE{} and \myHighlight{$b=b_2,$}\coordHE{} respectively).

{\bf Theorem.} {\it Quantum physical observables \myHighlight{$a$}\coordHE{} and \myHighlight{$b$}\coordHE{} are incompatible
(i.e., corresponding operators do not commute) iff the observable \myHighlight{$a$}\coordHE{} is incompatible
with the contexts \myHighlight{${\cal S}_j^b$}\coordHE{} or vice versa:}

\myHighlight{$\delta(a=a_i; {\cal S}; {\cal S}_j^b)\not=0$}\coordHE{} or \myHighlight{$\delta(b=b_i; {\cal S}; {\cal S}_j^a)\not=0.$}\coordHE{}

{\bf Proof.} Let \myHighlight{${\cal S}$}\coordHE{} be an arbitrary quantum context. Thus it can be represented
by a normalized vector \myHighlight{$\phi \in {\cal H}.$}\coordHE{} We have:
$$\coord{}\boxMath{
...
P_{{\cal S}_2}(a=a_i)]
 ...
 We have \myHighlight{$(\phi, \phi_j^b)= k_j e^{i \xi_j}, (\phi_j^b, \phi_i^a) = k_{ji} e^{i \xi_{ji}},$}\coordHE{}
 where \myHighlight{$k_j, k_{ij} \geq 0.$}\coordHE{} We get:
$$\coord{}\boxMath{
\delta(a=a_i; {\cal S}; {\cal S}_j^b)= 2 k_1 k_2 k_{1i} k_{2i} \cos \theta_i,
}{dollar}{0pt}\coordE{}$$
where \myHighlight{$\theta_i= \xi_2-\xi_1 + \xi_{2i}- \xi_{1i}.$}\coordHE{}

a). Let \myHighlight{$[\hat{a}, \hat{b}]=0.$}\coordHE{} Then \myHighlight{$k_{12}= k_{21}=0.$}\coordHE{}  Hence
\myHighlight{$\delta(a=a_i; {\cal S}; {\cal S}_j^b)=0.$}\coordHE{}

b). Let \myHighlight{$[\hat{a}, \hat{b}] \not =0.$}\coordHE{} Then \myHighlight{$k_{12}, k_{21}\not =0.$}\coordHE{}
Let \myHighlight{$k_1,k_2 >0$}\coordHE{} be arbitrary constants such that 
\myHighlight{$k_1^2+k_2^2=1.$}\coordHE{} We choose a context \myHighlight{${\cal S}$}\coordHE{} that is described
by the state:

\myHighlight{$\phi= \sqrt{k_1} e ^{i\xi_{21}} \phi_1^b +\sqrt{k_2} e ^{i\xi_{11}} \phi_2^b.$}\coordHE{}

Here \myHighlight{$\theta=0$}\coordHE{} and, hence,

\myHighlight{$\delta(a=a_i; {\cal S}; {\cal S}_j^b)=2 k_1 k_2 k_{i1} k_{i2} >0.$}\coordHE{}
