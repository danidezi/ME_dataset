...
The interaction of an electric charge \myHighlight{$Q$}\coordHE{} with a point dipole
\myHighlight{${\bf p}$}\coordHE{}, which we treat nonrelativistically and refer to as the
...
with \myHighlight{$m$}\coordHE{} being the reduced mass of the system and \myHighlight{$K_{e}$}\coordHE{} the
electrostatic constant. In Eq.~(\ref{eq:coupling}) the
characteristic dipole moment \myHighlight{$p_{0}$}\coordHE{} sets the scale for our
...
a polar molecule, \myHighlight{$Q=-e$}\coordHE{} (with \myHighlight{$e$}\coordHE{} being the electron charge magnitude), 
and \myHighlight{$p_{0}=e a_{0}/2 \approx 1.271 D$}\coordHE{}, 
where \myHighlight{$a_{0}$}\coordHE{} is the Bohr radius and \myHighlight{$D $}\coordHE{} is the
...
The corresponding classical Lagrangian \myHighlight{$L = m v^{2}/2
 - V({\bf r})
 $}\coordHE{} has an associated action that is
  invariant under the scale
transformations \myHighlight{$t \rightarrow \tau t$}\coordHE{}, \myHighlight{${\bf r} \rightarrow
\varrho {\bf r}$}\coordHE{}, (with \myHighlight{$\tau>0$}\coordHE{} and \myHighlight{$\varrho>0$}\coordHE{}), \myHighlight{$\varrho^{2}=
\tau$}\coordHE{}; this property is shared by the larger class of homogeneous
potentials of degree \myHighlight{$-2$}\coordHE{}~\cite{cam:00b}, which also includes the
...
generalized to~\cite{jac:80,jac:90} \myHighlight{$t \rightarrow \tilde{t} = t -
\alpha f(t) $}\coordHE{}, \myHighlight{${\bf r} \rightarrow \tilde{\bf r} (\tilde{t}) =
J^{\delta} \, {\bf r}(t)$}\coordHE{}, with \myHighlight{$J= |d \tilde{t}/dt|$}\coordHE{}. Invariance
of the action occurs only when the following two conditions are
simultaneously satisfied: (i) \myHighlight{$\delta=1/2$}\coordHE{} and (ii) \myHighlight{$f(t)$}\coordHE{} is
quadratic in \myHighlight{$t$}\coordHE{}. This selects the SO(2,1) conformal group, just
as for the inverse square potential~\cite{alf:76}, the magnetic
monopole~\cite{jac:80}, and the magnetic vortex~\cite{jac:90},
with the following three generators: (i) the Hamiltonian \myHighlight{$H$}\coordHE{},
Eq.~(\ref{eq:Hamiltonian}), associated with time translations \myHighlight{$t
\rightarrow t - \alpha $}\coordHE{}
 [for \myHighlight{$f(t) =1$}\coordHE{}];
...
paragraph, with \myHighlight{$\tau= 1 - \alpha$}\coordHE{} [for \myHighlight{$f(t)=t$}\coordHE{}]; and (iii) the
conformal generator
\begin{equation}\coord{}\boxEquation{
...
associated with the time special conformal transformation \myHighlight{$1/t
\rightarrow 1/t + \alpha$}\coordHE{} [for \myHighlight{$f(t)=t^{2}$}\coordHE{}].
 The corresponding
commutators \myHighlight{$[D,H]= -i \hbar H$}\coordHE{}, \myHighlight{$[D,K]= i \hbar K$}\coordHE{}, and \myHighlight{$[H,K]=
2 i \hbar D$}\coordHE{} show that these three operators form an SO(2,1)
algebra~\cite{wyb:74}.
...
dipole in spherical coordinates, with the separation of variables
 \myHighlight{$\Psi (r,\theta, \phi)= u(r) \Theta (\theta) e^{im\phi}/r$}\coordHE{},
where the azimuthal dependence corresponds to conservation of the
axial component \myHighlight{$L_{z}$}\coordHE{} of angular momentum. Then, the
corresponding equations for \myHighlight{$r$}\coordHE{} and \myHighlight{$\theta$}\coordHE{} are explicitly given
by
\begin{equation}\coord{}\boxEquation{
...
where \myHighlight{$\eta=2mE/\hbar^{2}$}\coordHE{},
\begin{equation}\coord{}\boxEquation{
...
and
  \myHighlight{$\Lambda^{2} =
L^{2} /\hbar^{2}$}\coordHE{} is the dimensionless angular momentum (squared).
...
coupling strength \myHighlight{$ \lambda_{\rm ISP} \equiv \gamma$}\coordHE{}. In addition,
...
or strong-coupling regime \myHighlight{$ \gamma \geq \gamma^{(*)}=1/4$}\coordHE{}
(for \myHighlight{$l=0$}\coordHE{} in three dimensions).
...
\myHighlight{$\gamma^{(*)}=1/4$}\coordHE{}, i.e.,
\begin{equation}\coord{}\boxEquation{
D(\gamma^{(*)},\lambda^{(*)}) =0 \; ,
...
shows that 
\myHighlight{$\lambda^{(*)} \approx 1.279$}\coordHE{},
which amounts to the familiar critical dipole moment \myHighlight{$p^{(*)} \approx
1.625 D$}\coordHE{}~\cite{fer:47,wig:49,lev:67,bro:67,cra:67}. Extensive
...
\myHighlight{$R_{\pm}$}\coordHE{} represents the distance to the charge \myHighlight{$Q$}\coordHE{} from the positive
and negative charges of the dipole, which are separated by a distance \myHighlight{$a$}\coordHE{}.
Equation~(\ref{eq:exact_finite_dipole_potential}) displays the
point-dipole potential~(\ref{eq:point_dipole_potential}), with
\myHighlight{$p=qa$}\coordHE{} (\myHighlight{$l=1$}\coordHE{}), supplemented by a symmetry-breaking potential
\myHighlight{$V_{\rm sb}({\bf r})$}\coordHE{}, which  includes higher-order multipoles
(for \myHighlight{$l>1$}\coordHE{} and \myHighlight{$r>a/2$}\coordHE{}, with moments \myHighlight{$\sim qa^{l}=p a^{l-1}$}\coordHE{}), as
well as the contribution to the potential for \myHighlight{$r<a/2$}\coordHE{}. In short,
...
as we will show next, this is indeed the case.
 In other words, even though the finite dipole introduces a length scale \myHighlight{$a$}\coordHE{}
...
value are independent of \myHighlight{$a$}\coordHE{}. In fact, this result is confirmed
...
finite dipole are \myHighlight{$\hbar$}\coordHE{}, \myHighlight{$m$}\coordHE{}, \myHighlight{$q$}\coordHE{}, and \myHighlight{$a$}\coordHE{}, as well as the
finite charge \myHighlight{$Q$}\coordHE{}; moreover the interaction only involves the
product \myHighlight{$Qq$}\coordHE{}.
...
and \myHighlight{$F(\lambda)$}\coordHE{} is an arbitrary function of \myHighlight{$\lambda$}\coordHE{}. On the
other hand, the critical value of the dimensionless coupling,
\myHighlight{$\lambda^{(*)}$}\coordHE{}, occurs when the ground state energy \myHighlight{$E_{\rm gs}$}\coordHE{}
...
\myHighlight{$a$}\coordHE{} of the dipole. In particular, the critical value survives in
the limit \myHighlight{$a \rightarrow 0$}\coordHE{}, which amounts to the ideal point
...
\bibitem{debye}
The debye \myHighlight{$D$}\coordHE{} is a characteristic scale for  molecular dipole
moments, defined to be exactly  \myHighlight{$1$}\coordHE{}  \myHighlight{$D = 10^{-18}$}\coordHE{} esu-cm.
...
The rapid growth in the value of \myHighlight{$|E_{\rm gs}|$}\coordHE{}
