...
\providecommand{\kbb}{{\boldsymbol k}}
\providecommand{\vb}{{\boldsymbol v}}
\providecommand{\bb}{{\boldsymbol v}}
\providecommand{\abb}{{\boldsymbol a}}
...
which only falls off as \myHighlight{$1/r$}\coordHE{}. This is too slow to be neglected and generates the infra-red problem (see below).
...
at asymptotically large times, \myHighlight{$t$}\coordHE{}.
...
where \myHighlight{$D(p,t)$}\coordHE{} is an (\myHighlight{$A_\mu$}\coordHE{}) field dependent
...
vanish asymptotically, the matter field in the Lagrangian is not gauge invariant, \myHighlight{$\psi\to \exp(-ie\theta)\psi$}\coordHE{},
...
that the combination, \myHighlight{$\mathrm{e}^{-ie\frac{\partial_iA_i}{\nabla^2}}\psi$}\coordHE{},
...
three dimensional Green's function for \myHighlight{$1/\nabla^2$}\coordHE{}. We would say that this is a description of a matter field
...
systematic approach to finding the correct \emph{dressing}. Writing our dressed field as \myHighlight{$h^{-1}\psi$}\coordHE{},
we have two requirements\cite{Bagan:1998kg}
on the dressing, \myHighlight{$h^{-1}$}\coordHE{} describing a particle moving with four-velocity \myHighlight{$u$}\coordHE{}:\\[2mm]
\begin{center}
\begin{minipage}{.97\linewidth}\begin{center}\fbox{\begin{minipage}{.87\linewidth}
\begin{center}
Local gauge invariance of \myHighlight{$h^{-1}\psi$}\coordHE{}: which implies \myHighlight{$h^{-1}\to h^{-1}U$}\coordHE{}\\[3mm]
An additional  \emph{dressing equation}: \myHighlight{$u\cdot \partial h^{-1}=-ieh^{-1}u\cdot A$}\coordHE{}\\[1mm]
\end{center}
...
\myHighlight{$u\cdot \partial (h^{-1}\Phi)=0$}\coordHE{}, and in the heavy effective
theory the equation of motion for the matter field has the form \myHighlight{$u\cdot D\Phi=0$}\coordHE{}. Combining
...
the combination \myHighlight{$h^{-1}\psi$}\coordHE{} is locally gauge invariant and can be identified with a charged particle.}
...
and the \myHighlight{$S$}\coordHE{}-matrix elements have IR divergences which are of two forms: soft divergences and (imaginary) phase
...
times in QED up to order \myHighlight{$e$}\coordHE{}:
\begin{equation}\coord{}\boxEquation{
...
where \myHighlight{$V^\mu=(\eta+v)^\mu(\eta-v)\cd k-k^\mu$}\coordHE{}, with \myHighlight{$u^\mu=\gamma(\eta+v)^\mu$}\coordHE{} being
