...
where \myHighlight{$P_\mu =(P_i, P_0)$}\coordHE{} are space and time components of
four-momentum, and \myHighlight{$M_{\mu\nu}$}\coordHE{} are deformed Lorentz generators consisting of
rotations \myHighlight{$M_k = \frac12\epsilon_{ijk}M_{ij} $}\coordHE{} and boosts
\myHighlight{$N_i = M_{0i}$}\coordHE{}. Moreover, \myHighlight{$\kappa$}\coordHE{} is a scale parameter, usually identified with the Planck mass (I use the units in which the universal speed of light, \myHighlight{$c=1$}\coordHE{}). Let us note that this parameter could be either positive or negative, leading to completely different physical picture. This can be seen from the form of the Casimir operator of this algebra, to wit
...
There are in principle many ways how one can extend the algebra (\ref{4}) along with (\ref{5}) to the whole of the phase space. However having in mind that we demand that all four components of momenta and positions \myHighlight{$x_i$}\coordHE{}   commute among themselves
$$\coord{}\boxMath{ [ p_\mu, p_\nu] =0, \quad [ x_i, x_j] =0, }{dollar}{0pt}\coordE{}$$ this freedom is drastically reduced. For example it follows from Jacobi identity that the commutators of \myHighlight{$\vec{p}$}\coordHE{} with \myHighlight{$\vec{x}$}\coordHE{}  cannot depend on \myHighlight{$p_i$}\coordHE{}, \myHighlight{$x_i$}\coordHE{}. One of the possible algebras (called ``Heisenberg-double'' and analyzed first in \cite{luno})
$$\coord{}\boxMath{
...
with all other brackets equal zero\footnote{One may wonder, why I put \myHighlight{$1$}\coordHE{} on the left hand side of (\ref{11}). The reason is that had I put a function \myHighlight{$g(p_0)$}\coordHE{} instead, it could be absorbed into redefinition \myHighlight{$x_0$}\coordHE{}.}. Let us investigate the Jacobi identity for this algebra. The only non-trivial identity is of the form
...
This completes the construction of the algebra. Now we will use it to deduce the way how the positions \myHighlight{$x$}\coordHE{} transform under action of boosts.



\section{The invariance of algebra: transformation\\ rules for \myHighlight{$x_i$}\coordHE{} and \myHighlight{$x_0$}\coordHE{}}
...
The procedure is rather straightforward: we use the known transformation rules for momenta (\ref{5}), the Leibniz rule, and try to discover what are the rules for transformation of \myHighlight{$x_0$}\coordHE{} and \myHighlight{$x_i$}\coordHE{}, so that the algebra is covariant with respect to boosts. Consider, for example, the bracket (\ref{7}). We have
...
where \myHighlight{$\ldots$}\coordHE{} denote terms which have vanishing bracket with \myHighlight{$p_0$}\coordHE{}, and the function \myHighlight{$A(p_0;\kappa)$}\coordHE{} must go to \myHighlight{$1$}\coordHE{} when \myHighlight{$p_0$}\coordHE{} is small as compared to \myHighlight{$\kappa$}\coordHE{}. 

By making use of the rotational symmetry one can find a most general expression for \myHighlight{$\delta_i x_0 $}\coordHE{} and \myHighlight{$\delta_i x_j $}\coordHE{}, linear in \myHighlight{$x$}\coordHE{} and then, after straightforward but rather tedious computations, the condition that the algebra (\ref{6}--\ref{8}) is to be covariant  along with the condition
...
Since the deformed boost transformations (\ref{11}), (\ref{12}) are linear, one can readily find the quadratic form in \myHighlight{$x_0$}\coordHE{}, \myHighlight{$\vec{x}$}\coordHE{}, invariant under them. Surprisingly enough, this  is just the Minkowski distance:
...
where \myHighlight{$p^{(0)}_0$}\coordHE{} and \myHighlight{$p^{(0)}$}\coordHE{} are initial values of energy and momentum, respectively, and  the parameter \myHighlight{$A$}\coordHE{} equals
...
Let us assume now that the particle is initially at rest (\myHighlight{$p^{(0)}=0$}\coordHE{}), and its rest energy \myHighlight{$p^{(0)}_0 =M$}\coordHE{}. Then
...
One can readily see that for small \myHighlight{$\xi$}\coordHE{}, \myHighlight{$z(\xi) \sim \xi$}\coordHE{} and the transformations (\ref{16}) become the standard Lorentz boosts. For large \myHighlight{$\xi$}\coordHE{}, on the other hand, the boosts saturate \myHighlight{$z(\xi) \sim const$}\coordHE{}, and both \myHighlight{$x_0(\xi)$}\coordHE{} and \myHighlight{$x(\xi)$}\coordHE{} become constant (in four dimensions in the large \myHighlight{$\xi$}\coordHE{} limit boosts become pure rotations.)
...
Having the structure of phase space (\ref{6} --\ref{8}) it is a straightforward exercise to derive equations of motion for free particles. The Poisson brackets algebra among phase space coordinates \myHighlight{$Y_A = (x_0, x_i; p_0, p_i)$}\coordHE{} provides us with the symplectic structure \myHighlight{$\omega_{AB}$}\coordHE{} and the equation of motion read
\begin{equation}\coord{}\boxEquation{\label{22}
 \frac{d\, Y_A}{d s} = \omega_{AB} \frac{\partial\, H}{\partial\,Y_B},
}{\frac{d\, Y_A}{d s} = \omega_{AB} \frac{\partial\, H}{\partial\,Y_B},
}{ecuacion}\coordE{}\end{equation}
where \myHighlight{$s$}\coordHE{} is the affine parameter. Similarly one can find the hamiltonian form action describing the free motion of a particle.
Both the equations and the action are by construction covariant (resp.~invariant) with respect to deformed transformation if the hamiltonian \myHighlight{$H$}\coordHE{}, used to describe the one-particle dynamics is  invariant with respect to these transformations. It is natural therefore (and consistent with the fact that in the limit \myHighlight{$\kappa\rightarrow\infty$}\coordHE{} equation (\ref{22}) should become the standard equation of special relativity) to choose the hamiltonian to be equal to the Casimir (\ref{2})
...
\begin{equation}\coord{}\boxEquation{\label{27}
 ...
 Therefore the massless particles move (in space-time) with the universal speed of light. This result seems quite alarming if one recalls the expression for \myHighlight{${\cal C}$}\coordHE{}, (\ref{3}). This puzzle may be however solved as follows. The formula (\ref{3}) describes the group velocity of the wave, while (\ref{27}) corresponds to its phase velocity. In the standard optics these two velocities differ as a result of the dependence of the refraction index on frequency. Here the situation is quite different. The apparent discrepancy between these two expressions arises if one assumes that the relation of energy \myHighlight{$p_0$}\coordHE{} and frequency \myHighlight{$\omega$}\coordHE{} and momentum \myHighlight{$p$}\coordHE{} and wavelenght \myHighlight{$\lambda$}\coordHE{} is standard, as in special relativity. But this does not necessarily need to be  the case here. One can easily check that if one takes
...
in complete agreement with eq.~(\ref{27}). It should be stressed  that what was said above is just a  claim (very plausible, however), which will have to be justified by more solid arguments, for example by analyzing propagation of waves and investigating the particle--wave duality in depth. In other words, equations (\ref{27a}) -- (\ref{27c}) will have to be derived from some first principles.
 ...
 Let us investigate now the behavior of massive particles. Consider a particle at rest with the initial value of energy \myHighlight{$p_0=M$}\coordHE{}. If we boost this particle with the rapidity \myHighlight{$\xi$}\coordHE{}, its momentum and energy will be equal \cite{rbgacjkg}
...
Observe that in this way one obtains a measure of the relative velocities of two observers \myHighlight{$V$}\coordHE{} as a function of the rapidity parameter \myHighlight{$\xi$}\coordHE{}. Let us analyze this expression in the limit of  boosts going to infinity. One gets
...
To derive the contraction formula, consider two particles (of  masses \myHighlight{$M_1$}\coordHE{} and \myHighlight{$M_2$}\coordHE{}) at rest in one coordinate system: the first in position \myHighlight{$x^{(1)} =0$}\coordHE{} and the second in position \myHighlight{$x^{(2)}=l$}\coordHE{}. Solving their equation of motion one gets
...
If we now boost the particle \myHighlight{$(1)$}\coordHE{} its motion will be described by
...
The distance between the particles measured by boosted observer \myHighlight{$l'$}\coordHE{} will be equal to the difference between \myHighlight{$x^{(2)}[\xi;\bar s]$}\coordHE{} and \myHighlight{$x^{(1)}[\xi; s]$}\coordHE{} with \myHighlight{$s$}\coordHE{} and \myHighlight{$\bar s$}\coordHE{} such that \myHighlight{$x^{(1)}_0[\xi;s] =  x^{(2)}_0[\xi;\bar s]$}\coordHE{}. The simple computation shows that

\begin{equation}\coord{}\boxEquation{\label{35}
 l' = l\cosh(z(\xi)) - l \frac{\sinh(z(\xi))^2}{\cosh(z(\xi))} = l\frac{1}{\cosh(z(\xi))}
}{l' = l\cosh(z(\xi)) - l \frac{\sinh(z(\xi))^2}{\cosh(z(\xi))} = l\frac{1}{\cosh(z(\xi))}
}{ecuacion}\coordE{}\end{equation}
which is exactly the expected result. Observe that since the function \myHighlight{$z(\xi)$}\coordHE{} goes to the constant limit when \myHighlight{$\xi\rightarrow\infty$}\coordHE{}, the fraction \myHighlight{$l/l'$}\coordHE{} is, contrary to special relativity, limited from above. 
...
\item It is proved that the metric structure of so constructed ``space-time of events'' possesses the Minkowski distance as an invariant quadratic form. This means that this space is \myHighlight{$R^4$}\coordHE{} equipped with Minkowski metric (contrary to the energy-momentum space, for whose the invariant form is the Casimir, (\ref{2}).
\item Comparing the formulas for velocities \myHighlight{${\cal C}$}\coordHE{} (\ref{3}) and the one following the Minkowski structure of the space-time of events I propose the relation between energy and momentum on the one side and frequency and inverse wavelength, on the other (\ref{27a}), (\ref{27b}).
