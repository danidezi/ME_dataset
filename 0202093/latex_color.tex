...
\providecommand{\authors}[1]{\noindent{\large #1}\vspace{15mm}}
\providecommand{\address}[1]{{\center{\noindent #1\vspace{0mm}}}}
...
\authors {  \Large{J. M. Grimstrup\myHighlight{$^{1}$}\coordHE{}, H. Grosse\myHighlight{$^{2}$}\coordHE{}, L. Popp\myHighlight{$^{3}$}\coordHE{}, V. Putz\myHighlight{$^{4}$}\coordHE{}, M.~Schweda\myHighlight{$^{5}$}\coordHE{}, M. Wickenhauser\myHighlight{$^{6}$}\coordHE{}, R. Wulkenhaar\myHighlight{$^{7}$}\coordHE{} }}    \vspace{-20mm}

\vspace{10mm}
       
\address{\myHighlight{$^{1,3,5,6}$}\coordHE{}  Institut f\"ur Theoretische Physik, Technische Universit\"at Wien\\
      Wiedner Hauptstra\ss e 8--10, A-1040 Wien, Austria}
\address{\myHighlight{$^{2}$}\coordHE{}  Institut f\"ur Theoretische Physik, Universit\"at Wien\\Boltzmanngasse 5, A-1090 Wien, Austria   }
\address{\myHighlight{$^{4,7}$}\coordHE{}} Max-Planck-Institute for Mathematics in the Sciences \\
...
In this letter we discuss an alternative approach to the problem of the so-called UV/IR-mixing in noncommutative Euclidean scalar \myHighlight{$\phi^4$}\coordHE{}-theory. The classical  action is given by 
...
Therefore, the original UV-divergence is replaced by an IR-singularity in the limit of vanishing \myHighlight{$p$}\coordHE{} implying \myHighlight{$\Lambda_{\mathrm{eff}} \rightarrow \infty$}\coordHE{} (for a rigorous analysis see \cite{Chepelev:2000hm}).
...
In this section we briefly review the well known results of \cite{Minwalla:1999px}, \cite{VanRaamsdonk:2000rr} showing that the one-loop nonplanar graphs in the scalar noncommutative field theory on four-dimensional Euclidean space are convergent at generic values of external momenta due to rapid oscillations of the phase factors \myHighlight{$\mathrm{e}^{\im p \times k}$}\coordHE{}, where \myHighlight{$p$}\coordHE{} is an external momentum, \myHighlight{$k$}\coordHE{} the loop momentum, and \myHighlight{$k \times p = k_\mu \theta^{\mu\nu} p_\nu$}\coordHE{}. 
...
For \myHighlight{$\theta=0$}\coordHE{} the above integrals are identical up to a factor 2.
...
where \myHighlight{$M$}\coordHE{} is the renormalized physical mass and \myHighlight{$\delta m^2$}\coordHE{} corresponds to a regularized planar graph and diverges quadratically if the cutoff tends to infinity. The mass counterterm \myHighlight{$\delta m^2$}\coordHE{} cancels just the divergence of the first integral in \eqref{seibergint}.
...
one can easily calculate the resulting Gaussian integrals. Regularizing by multiplication with \myHighlight{$\mathrm{e}^{\frac{-1}{\Lambda^2 \alpha}}$}\coordHE{} one gets the following results:
\begin{align}\coord{}\boxAlignEqnarray{
  \label{seires}
...
In \eqref{eq:lambdaeff} we have introduced the notation \myHighlight{$\tilde{p}^\mu = \theta^{\mu\nu} p_\nu$}\coordHE{}.
...
As is explained in \cite{Minwalla:1999px} the limit \myHighlight{$\Lambda \rightarrow \infty$}\coordHE{} does not commute with the low momentum limit \myHighlight{$p \rightarrow 0$}\coordHE{} (IR-region)---the so-called \emph{UV/IR-mixing} of noncommutative quantum field theories.
...
(without the factor \myHighlight{$\tfrac{1}{\tilde{p}^2 + \tfrac{1}{\Lambda^2}}$}\coordHE{}) as a result of a field redefinition
\begin{equation}\coord{}\boxEquation{
  \phi(p) \rightarrow \phi(p) + f(p, \theta, \Lambda) \phi(p).
}{
  \phi(p) \rightarrow \phi(p) + f(p, \theta, \Lambda) \phi(p).
}{ecuacion}\coordE{}\end{equation}
A simple calculation shows that a solution \myHighlight{$f(p)$}\coordHE{} is of the following form
\begin{equation}\coord{}\boxEquation{
...
whence the redefinition of the field \myHighlight{$\phi(x)$}\coordHE{} in position space
\begin{equation}\coord{}\boxEquation{
  \label{xredef}
...
It is now straightforward to compute an ``IR-regular'' quadratic effective action up to the given order in \myHighlight{$g^2$}\coordHE{} with this new action, yielding
\begin{multline}\coord{}\boxAlignEqnarray{
...
\item The correction term in the field redefinition \eqref{xredef} is of order \myHighlight{$g^2$}\coordHE{}. Thus, the bare propagator (the free-field case being defined by \myHighlight{$g=0$}\coordHE{}) remains unchanged:
  \begin{equation}\coord{}\boxEquation{
...
\item Since the dangerous term \myHighlight{$\frac{\hbar g^2}{96 \pi^2} \frac{1}{\tilde{p}^2 + \frac{1}{\Lambda^2}}$}\coordHE{} in \eqref{gammaeff} does not depend on the mass (physical or bare mass) the massless case is also well defined implying a field redefinition
  \begin{equation}\coord{}\boxEquation{
    \phi(x) \rightarrow  \phi(x) - \frac{1}{2} \frac{\hbar g^2}{96 \pi^2} 
