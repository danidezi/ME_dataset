...
\providecommand{\cyc}[1]{\mathbb{Q}\left[ \zeta_{#1 }\right] }



\providecommand{\Mod}[3]{#1\equiv#2\, \left(\mathrm{mod}\, \, #3\right)}



\providecommand{\FA}[1]{N}
 


\providecommand{\FB}[1]{\mathbb{P}\mathcal{K}}



\providecommand{\FC}[1]{\mathcal{G}_{#1}}



\providecommand{\FD}[1]{\Gamma\left(K,\mathfrak{#1}\right)}



\providecommand{\FE}[2]{\ker\pi}
...
primary fields under the modular group \myHighlight{$\Gamma\left(1\right)=SL\left(2,\mathbb{Z}\right)$}\coordHE{},
...
there exists a natural number \myHighlight{$\FA{}$}\coordHE{} (which turns out to equal the
order of the matrix \myHighlight{$T$}\coordHE{} representing the Dehn-twist \myHighlight{$\tau\mapsto\tau+1$}\coordHE{}),
such that the kernel of the modular representation contains the principal
congruence subgroup \[\coord{}\boxMath{
\Gamma\left(\FA{}\right)=\set{\left(\begin{array}{cc}
a & b\\
c & d\end{array}\right)\in\Gamma\left(1\right)}{\Mod{a,d}{1}{\FA{}},\,\,\Mod{b,c}{0}{\FA{}}}\,.}{corchetes}{0pt}\coordE{}\]
In other words all modular transformations which belong to \myHighlight{$\Gamma\left(\FA{}\right)$}\coordHE{}
...
a finite dimensional representation \myHighlight{$D$}\coordHE{} of the modular group \myHighlight{$\Gamma\left(1\right)$}\coordHE{}.
This representation is most conveniently described by a pair of matrices
\myHighlight{$S$}\coordHE{} and \myHighlight{$T$}\coordHE{}, which represent the generators \myHighlight{$\left(\begin{array}{cc}
0 & -1\\
1 & 0\end{array}\right)$}\coordHE{} and \myHighlight{$\left(\begin{array}{cc}
1 & 1\\
0 & 1\end{array}\right)$}\coordHE{} of the modular group. These matrices have to satisfy the defining
relations \begin{eqnarray*}\coord{}\boxAlignEqnarray{\leftCoord{}
STS & = & T^{-1}ST^{-1}\rightCoord{}\,,\rightCoord{}\\\leftCoord{}
S^{4} & = & 1}{0mm}{2}{3}{
STS & = & T^{-1}ST^{-1}\,,\\
S^{4} & = & 1}{1}\coordE{}\end{eqnarray*}
of the modular group. It should be noted that the matrices \myHighlight{$S$}\coordHE{} and
\myHighlight{$T$}\coordHE{} carry much more information than a mere linear representation
of \myHighlight{$\Gamma\left(1\right)$}\coordHE{}, for the individual matrix elements have
...
\begin{enumerate}
\item The matrix \myHighlight{$T$}\coordHE{} is diagonal and of finite order;
\item The matrix \myHighlight{$S$}\coordHE{} is symmetric;
\item The fusion rules of the primaries are given by Verlinde's formula
\[\coord{}\boxMath{
N_{pqr}=\sum_{s}\frac{S_{p}^{s}S_{q}^{s}S_{r}^{s}}{S_{0}^{s}}\,,}{corchetes}{0pt}\coordE{}\]
where \myHighlight{$0$}\coordHE{} labels the vacuum (the conformal block of the identity).
\end{enumerate}
...
mentioned in the introduction, i.e. that the kernel \myHighlight{$\ker D$}\coordHE{} of the
modular representation is a congruence subgroup of level \myHighlight{$\FA{}$}\coordHE{}
\cite{CMP}. One may also show that the \emph{conductor} \myHighlight{$\FA{}$}\coordHE{},
which is equal to the order of the matrix \myHighlight{$T$}\coordHE{}, is bounded by a universal
function of the dimension (= number of primary fields). Moreover,
if we denote by \myHighlight{$K$}\coordHE{} the projective order of \myHighlight{$T$}\coordHE{}, i.e. the smallest
positive integer such that \myHighlight{$T^{K}$}\coordHE{} is a scalar matrix, then the integer
\myHighlight{$e=\frac{\FA{}}{K}$}\coordHE{} is a divisor of \myHighlight{$12$}\coordHE{} \cite{CMP}. The integers
\myHighlight{$e$}\coordHE{} and \myHighlight{$K$}\coordHE{} are important characteristics of the model.
...
basic idea is to look at the field \myHighlight{$F$}\coordHE{} obtained by adjoining to the
rationals the matrix elements of all modular matrices (it is enough
to adjoin those of the generators \myHighlight{$S$}\coordHE{} and \myHighlight{$T$}\coordHE{}). It turns out that
the field \myHighlight{$F$}\coordHE{} equals the cyclotomic field \myHighlight{$\cyc{\FA{}}$}\coordHE{}, where \myHighlight{$\FA{}$}\coordHE{}
is the conductor of the model. By a well known result of algebraic
number theory, the Galois group of the cyclotomic field \myHighlight{$\cyc{\FA{}}$}\coordHE{}
over \myHighlight{$\mathbb{Q}$}\coordHE{} is isomorphic to the group \myHighlight{$\FC{\FA{}}=\left(\mathbb{Z}/NZ\right)^{*}$}\coordHE{}
of prime residues modulo \myHighlight{$\FA{}$}\coordHE{}, where to the prime residue \myHighlight{$l\in\FC{\FA{}}$}\coordHE{}
corresponds the map \myHighlight{$\sigma_{l}$}\coordHE{} sending \myHighlight{$\zeta_{\FA{}}$}\coordHE{} to its
\myHighlight{$l$}\coordHE{}-th power \myHighlight{$\zeta_{\FA{}}^{l}$}\coordHE{}. As \myHighlight{$\sigma_{l}$}\coordHE{} maps \myHighlight{$F$}\coordHE{} into
itself, it is meaningful to consider its action on the matrix elements
of \myHighlight{$S$}\coordHE{} and \myHighlight{$T$}\coordHE{}. The basic result \cite{BG,CG1}, which follows from
Verlinde's theorem, is \[\coord{}\boxMath{
\sigma_{l}\left(S_{q}^{p}\right)=\varepsilon_{l}\left(q\right)S_{\pi_{l}q}^{p}\,,}{corchetes}{0pt}\coordE{}\]
where \myHighlight{$\pi_{l}$}\coordHE{} is a permutation of the primaries - the \emph{Galois
permutation} associated to \myHighlight{$l\in\FC{\FA{}}$}\coordHE{} -, while \myHighlight{$\varepsilon_{l}\left(q\right)=\pm1$}\coordHE{}
is a sign. The above Galois permutations define a permutation action
of \myHighlight{$\FC{\FA{}}$}\coordHE{} on the set of primaries, because they satisfy \myHighlight{$\pi_{lm}=\pi_{l}\pi_{m}$}\coordHE{}.


\section{Galois currents}

According to the traditional definition \cite{sc1,sc2,sc3}, a simple
current is a primary field \myHighlight{$\alpha$}\coordHE{} whose quantum dimension is \myHighlight{$1$}\coordHE{},
i.e. such that \myHighlight{$S_{0}^{\alpha}=S_{0}^{0}$}\coordHE{}. From this definition follows
that the fusion product of a simple current \myHighlight{$\alpha$}\coordHE{} with a primary
\myHighlight{$p$}\coordHE{} is again a primary field, denoted \myHighlight{$\alpha p$}\coordHE{}, i.e. simple currents
...
simple current \myHighlight{$\alpha$}\coordHE{} the set \[\coord{}\boxMath{
L_{\alpha}=\set{l\in\FC{\FA{}}}{\alpha p=\pi_{l}p}\,,}{corchetes}{0pt}\coordE{}\]
i.e. the set of prime residues mod \myHighlight{$\FA{}$}\coordHE{} such that the Galois permutation
\myHighlight{$\pi_{l}$}\coordHE{} is the same as the permutation induced by \myHighlight{$\alpha$}\coordHE{} (this
set may be empty). We call a simple current \myHighlight{$\alpha$}\coordHE{} a \emph{Galois
current} when \myHighlight{$L_{\alpha}\neq\emptyset$}\coordHE{}. The set of Galois currents
...
\myHighlight{$1\in L_{0}$}\coordHE{}. The interesting question is whether there exists models
with non-trivial simple currents, so let's give a couple of simple
examples.

Our first example is the Ising model, the Virasoro minimal model \myHighlight{$\mathcal{M}\left(4,3\right)$}\coordHE{}
of central charge \myHighlight{$c=\frac{1}{2}$}\coordHE{}. As it is well known, this model
has 3 primaries: the vacuum \myHighlight{$0$}\coordHE{}, the energy operator \myHighlight{$\varepsilon$}\coordHE{},
and the spin operator \myHighlight{$\sigma$}\coordHE{}. One has \myHighlight{$K=16$}\coordHE{} and \myHighlight{$e=3$}\coordHE{} in this
model. A simple computation reveals that \myHighlight{$L_{\varepsilon}=\left\{ 5,11,13,19,29,35,37,43\right\} $}\coordHE{},
i.e. \myHighlight{$\varepsilon$}\coordHE{} is a non-trivial Galois current. More generally,
one may show that the only Virasoro minimal models with a non-trivial
Galois current are those of the form \myHighlight{$\mathcal{M}\left(4,p\right)$}\coordHE{}
with \myHighlight{$p$}\coordHE{} odd - the Ising model corresponding to \myHighlight{$p=3$}\coordHE{} -, in which
case the primary field with Kac-label \myHighlight{$\left(3,1\right)$}\coordHE{} is the non-trivial
Galois current (for \myHighlight{$p>3$}\coordHE{}).

There are many more examples of theories with non-trivial Galois currents,
e.g. some of the Ashkin-Teller models (= \myHighlight{$\mathbb{Z}_{2}$}\coordHE{} orbifolds
...
\item If a theory has a non-trivial Galois current, then \myHighlight{$e$}\coordHE{} is odd and
\myHighlight{$K$}\coordHE{} is a multiple of \myHighlight{$16$}\coordHE{}.
\end{enumerate}
With respect to this last result, we note that the reverse implication
is by no means true, for there exist examples of RCFTs with odd \myHighlight{$e$}\coordHE{}
and \myHighlight{$K$}\coordHE{} a multiple of \myHighlight{$16$}\coordHE{}, but no non-trivial Galois current. On
the other hand, in those classes of RCFTs for which one has an explicit
expression for \myHighlight{$e$}\coordHE{} and \myHighlight{$K$}\coordHE{}, those with odd \myHighlight{$e$}\coordHE{} and \myHighlight{$K$}\coordHE{} a multiple
of \myHighlight{$16$}\coordHE{} are indeed sparse, in accord with the second observation
above.
...
the kernel is a congruence subgroup whose level equals the order \myHighlight{$\FA{}$}\coordHE{}
of \myHighlight{$T$}\coordHE{}. Knowing this fundamental result, the next natural step is
...
\emph{projective kernel} \myHighlight{$\FB{}=\set{m\in\Gamma\left(1\right)}{D\left(m\right)=\xi\left(m\right)1}$}\coordHE{},
which consists of those modular transformations that are represented
by scalar matrices, where \myHighlight{$\xi:\FB{}\rightarrow\mathbb{C}$}\coordHE{} is a linear
character of \myHighlight{$\FB{}$}\coordHE{}, which we term the \emph{central character}..
Let's explain why this is so.

Consider a composite system denoted \myHighlight{$\mathcal{C}_{1}\otimes\mathcal{C}_{2}$}\coordHE{},
which is made up of two independent and non-interacting subsystems
\myHighlight{$\mathcal{C}_{1}$}\coordHE{} and \myHighlight{$\mathcal{C}_{2}$}\coordHE{}. Note that if both subsystems
\myHighlight{$\mathcal{\mathcal{C}}_{1}$}\coordHE{}and \myHighlight{$\mathcal{C}_{2}$}\coordHE{} are RCFTs, then
so is their composite \myHighlight{$\mathcal{C}_{1}\otimes\mathcal{C}_{2}$}\coordHE{}. As
...
is ''natural'', for the modular representation associated to \myHighlight{$\mathcal{C}_{1}\otimes\mathcal{C}_{2}$}\coordHE{}
is the tensor product of the modular representations associated to
\myHighlight{$\mathcal{C}_{1}$}\coordHE{} and \myHighlight{$\mathcal{C}_{2}$}\coordHE{}. On the other hand, the
...
the parameter \myHighlight{$K$}\coordHE{} and not the conductor \myHighlight{$N$}\coordHE{} that enters most of
...
kernel. To this end, let's first introduce the notation \[\coord{}\boxMath{
\FD{g}=\left\{ \left(\begin{array}{cc}
a & b\\
c & d\end{array}\right)\in\Gamma\left(1\right)\,|\, a,d\in\mathfrak{g},\,\,\Mod{b,c}{0}{K}\right\} }{corchetes}{0pt}\coordE{}\]
for a subgroup \myHighlight{$\mathfrak{g}<\FC{K}$}\coordHE{}. Clearly, \myHighlight{$\Gamma\left(K,\left\{ 1\right\} \right)$}\coordHE{}
equals the principal congruence subgroup \myHighlight{$\Gamma\left(K\right)$}\coordHE{},
from which follows that \myHighlight{$\FD{g}$}\coordHE{} is a congruence subgroup for any
\myHighlight{$\mathfrak{g}<\FC{K}$}\coordHE{}. 

Remember the Galois permutations \myHighlight{$\pi_{l}$}\coordHE{} from section 2, and consider
the kernel of the permutation action \myHighlight{$\pi$}\coordHE{}, i.e. the set \myHighlight{$\FE{}{}=\set{l\in\FC{\FA{}}}{\pi_{l}=1}$}\coordHE{}.
Clearly, \myHighlight{$\FE{}{}$}\coordHE{} is a subgroup of \myHighlight{$\FC{\FA{}}$}\coordHE{}, and reducing all
elements of \myHighlight{$\FE{}{}$}\coordHE{} modulo \myHighlight{$K$}\coordHE{} we get a subgroup \myHighlight{$\mathfrak{h}$}\coordHE{}
of \myHighlight{$\FC{K}$}\coordHE{}. It does follow from general properties of the Galois
action that the exponent of \myHighlight{$\mathfrak{h}$}\coordHE{} is \myHighlight{$2$}\coordHE{}, in other words
\myHighlight{$\Mod{l^{2}}{1}{K}$}\coordHE{} for all \myHighlight{$l\in\mathfrak{h}$}\coordHE{} \cite{CMP}. The
basic result about the projective kernel is that \myHighlight{$\FD{h}$}\coordHE{} is a normal
subgroup of \myHighlight{$\FB{}$}\coordHE{} - in particular \myHighlight{$\FB{}$}\coordHE{} is a congruence subgroup
of level \myHighlight{$K$}\coordHE{} -, and there is an explicit isomorphism between the
factor group \[\coord{}\boxMath{
\FB{}/\FD{h}}{corchetes}{0pt}\coordE{}\]
and the group of Galois currents! This means that the knowledge of
\myHighlight{$K$}\coordHE{}, the subgroup \myHighlight{$\mathfrak{h}$}\coordHE{} and the group of Galois currents
does completely determine the projective kernel. Note that the index
of the principal congruence subgroup \myHighlight{$\Gamma\left(K\right)$}\coordHE{} in \myHighlight{$\FB{}$}\coordHE{},
which equals the number of Galois currents times the order of \myHighlight{$\mathfrak{h}$}\coordHE{},
is always a power of \myHighlight{$2$}\coordHE{} by the above, a result that seems fairly
non-trivial.

As an example, let's consider once again the Ising model \myHighlight{$\mathcal{M}\left(4,3\right)$}\coordHE{}.
As discussed in section 3, one has \myHighlight{$e=3$}\coordHE{} and \myHighlight{$K=16$}\coordHE{}, and there is
a non-trivial Galois current (the energy operator \myHighlight{$\varepsilon$}\coordHE{}).
In this case, the group \myHighlight{$\mathfrak{h}$}\coordHE{} consists of the prime residues
\myHighlight{$\left\{ 1,7,9,15\right\} $}\coordHE{} modulo \myHighlight{$16$}\coordHE{}. As to the projective kernel,
it has the coset decomposition \[\coord{}\boxMath{
\FB{}=\FD{h}\cup\left(\begin{array}{cc}
3 & 8\\
8 & 11\end{array}\right)\FD{h}}{corchetes}{0pt}\coordE{}\]
...
by an understanding of the central character, the map \myHighlight{$\xi:\FB{}\rightarrow\mathbb{C}$}\coordHE{}
appearing in the definition of the projective kernel. Another interesting
point would be to find out which subgroups \myHighlight{$\mathfrak{h}<\FC{K}$}\coordHE{}
are allowed for a given value of \myHighlight{$K$}\coordHE{}.
