...
NS-NS 2-form \myHighlight{$B$}\coordHE{}-field along the world-volume of either type of  D-branes, 
the world-volume
...
Besides the \myHighlight{$B$}\coordHE{} field, 
we can turn on different magnetic fluxes \myHighlight{$F$}\coordHE{} on the D-branes and then the
non-commutative field theories on the D-branes have multiple 
\myHighlight{$\theta$}\coordHE{} parameters related to the different values of \myHighlight{$F + B$}\coordHE{} 
\cite{ot5,nappi,dasy}.
...
background independent way. For D-branes with \myHighlight{$B$}\coordHE{} and \myHighlight{$F$}\coordHE{} fields, this 
includes a factor of \myHighlight{$\frac{B}{F+B}$}\coordHE{} in front of the usual coupling between 
the \myHighlight{$B$}\coordHE{} and \myHighlight{$F$}\coordHE{} fields and the RR-forms in type II theories. 
...
The rotation of the brane is related to the value of \myHighlight{$B + F$}\coordHE{} and the 
deformation of the torus is related to the value of \myHighlight{$B$}\coordHE{}.  
...
on a \myHighlight{$T^2$}\coordHE{} torus, with NS 2-form \myHighlight{$B$}\coordHE{}  and magnetic flux \myHighlight{$F$}\coordHE{} turned on the
...
to take \myHighlight{$F + B = Q^{-1}$}\coordHE{} constant and to allow a variable value for 
\myHighlight{$B = \theta^{-1}$}\coordHE{}. In this case the Born-Infeld and Chern-Simons actions could
be written as:
...
where \myHighlight{$T_9$}\coordHE{} and \myHighlight{$\mu_9$}\coordHE{} are the tension and charge of the D9 brane. In the
case of only 2 directions of B and F fields, we can write 
\myHighlight{$\mbox{Pf}~Q = Q$}\coordHE{} and \myHighlight{$\mbox{Pf}~\theta = \theta$}\coordHE{} 
...
Let us consider that the directions of the two-torus \myHighlight{$T^2$}\coordHE{} and of the \myHighlight{$B$}\coordHE{} and 
\myHighlight{$F$}\coordHE{} fields are on the \myHighlight{$(x^1, x^2)$}\coordHE{} plane. Therefore, if we want to
discuss the action of T-duality, we need to distinguish between T-duality on
the compact \myHighlight{$(x^1, x^2)$}\coordHE{} directions and T-dualities with respect to any other 
possible compact direction. 

If we consider \myHighlight{$x^3$}\coordHE{} as a compact direction and take a T-duality with respect
to it, then the D9 brane will become a D8 brane wrapped on  \myHighlight{$(x^1, x^2, x^3)$}\coordHE{},
...
where \myHighlight{$T_8$}\coordHE{} and \myHighlight{$\mu_8$}\coordHE{} are the tension and charge of the D8 brane.
In this case, the 9-th component of the BI vector \myHighlight{$A_{3}$}\coordHE{} gets mapped 
into the 
transverse dimension \myHighlight{$\phi_{3}$}\coordHE{} to the D8 brane in the usual sense.

What happens now if we consider a T-duality with respect to \myHighlight{$x^1$}\coordHE{} or \myHighlight{$x^2$}\coordHE{}
directions? As discussed in 
\cite{chen,ima,blu1,blu2,blu3}, there are two things
which happen: the \myHighlight{$T^2$}\coordHE{} torus gets deformed and the D8 branes is rotated in
the  \myHighlight{$(x^1, x^2)$}\coordHE{} plane. If we consider a T-duality along the 
\myHighlight{$x^2$}\coordHE{} direction, we obtain a torus with a rotation of the \myHighlight{$x^2$}\coordHE{} axis
into an \myHighlight{$x'^2$}\coordHE{} axis by an angle \myHighlight{$\pi/2 - \alpha$}\coordHE{} given by:
\begin{equation}\coord{}\boxEquation{
\label{angles1}
...
(so it makes and angle \myHighlight{$\alpha$}\coordHE{} with the \myHighlight{$x^1$}\coordHE{} axis) 
and the D8 brane has a direction on the deformed torus \myHighlight{$(x^1, x'^2)$}\coordHE{} 
at an angle \myHighlight{$\beta$}\coordHE{} with respect to the direction \myHighlight{$x^1$}\coordHE{} where
...
\myHighlight{$C^{(10)} + C^{(8)} \wedge (F + B)$}\coordHE{} when the \myHighlight{$F, B$}\coordHE{} are 
in the \myHighlight{$(x^1, x^2)$}\coordHE{} directions and
\myHighlight{$\hat C^{(8)}$}\coordHE{} is in the \myHighlight{$(x^0,x^3\cdots,x^9)$}\coordHE{} directions. 
Together with the factor \myHighlight{$\frac{Q}{\theta}$}\coordHE{} this will give
\begin{equation}\coord{}\boxEquation{
\label{d9}
...
A double dimensional reduction of the D9 brane on the \myHighlight{$x^2$}\coordHE{} direction gives a 
commutative D8 brane in the \myHighlight{$x^1,\cdots,x^{10}$}\coordHE{} directions with 
...
the component of the BI vector in the direction \myHighlight{$x^2$}\coordHE{} over which we reduce.

We then consider a direct dimensional reduction of a commutative D8 brane 
on the  \myHighlight{$(x'^{2},\cdots,x^{10})$}\coordHE{} directions whose Chern-Simons term in 
10 dimensions does not contain the   \myHighlight{$Q \over \theta$}\coordHE{} term in the action.
...
where \myHighlight{$\chi' = \Phi^{2}$}\coordHE{} comes from the reduction on the transverse direction.

The integral in (\ref{dr}) is taken over \myHighlight{$d x^1$}\coordHE{} and in (\ref{dir}) is taken
over \myHighlight{$d x'^2$}\coordHE{}.  But \myHighlight{$\frac{d x'^2}{d x^1}$}\coordHE{} is just \myHighlight{$\mbox{tan}~\alpha$}\coordHE{} so we
see that we see the appearance of \myHighlight{$\mbox{tan}~\alpha$}\coordHE{} in (\ref{dir}).
Moreover, the directly dimensional reduced D8 brane is at an angle 
\myHighlight{$\beta$}\coordHE{} with respect to \myHighlight{$x^1$}\coordHE{} so there is a factor \myHighlight{$\mbox{cot}~\beta$}\coordHE{} also
appearing in the action. Therefore we have a mapping between 
the 2-nd component of the BI vector and the extra transverse 
direction.

What happens if we take two T-dualities in the \myHighlight{$x^1, x^2$}\coordHE{} directions?
The D9 brane becomes a commutative D7 branes. 
In \cite{chu,blu1} is has been argued that the coordinates
\myHighlight{$x^1, x^2$}\coordHE{} of the D7 brane do not commute in this case. 
By turning on \myHighlight{$B_{12}$}\coordHE{} field and keeping the other 8 directions non-compact,
...
the large \myHighlight{$F, B$}\coordHE{} limit as \myHighlight{$[x_1, x_2] = 1/(F+B) = Q$}\coordHE{} so if we identify
\myHighlight{$\phi^{1} = x^1, \phi^{2} = x^2$}\coordHE{}, the Myers coupling becomes
 \myHighlight{$Q_{12}~C_{12 i_1 \cdots i_8}$}\coordHE{}. 
There is also a term \myHighlight{$1/\theta$}\coordHE{} coming
...
If we now consider the term \myHighlight{$\frac{Q}{\theta}~C^{(10)}$}\coordHE{} in the D9 brane 
...
seen as \myHighlight{$T^2 \times T^2$}\coordHE{} on the \myHighlight{$x^1, x^2$}\coordHE{} and \myHighlight{$x^3, x^4$}\coordHE{} directions 
respectively. We turn on \myHighlight{$B$}\coordHE{} and \myHighlight{$F$}\coordHE{} fields such that 
\myHighlight{$B_{12} = B_{1},~F_{12} = F_{1}$}\coordHE{} and 
\myHighlight{$B_{34} = B_{2},~F_{34} = F_{2}$}\coordHE{}.
Then we define 
\begin{equation}\coord{}\boxEquation{
...
and we insert in formula (\ref{cs1}) \myHighlight{$\mbox{Pf}~Q = Q_1~Q_2$}\coordHE{}
and \myHighlight{$\mbox{Pf}~\theta = \theta_1~\theta_2$}\coordHE{}. 

We then consider two T-dualities
in the \myHighlight{$x_2$}\coordHE{} and \myHighlight{$x_4$}\coordHE{} directions. Under T-duality, the D9 brane goes 
into a D7 brane rotated in the \myHighlight{$x^1, x^2$}\coordHE{} plane at an angle
\myHighlight{$\beta_{1}$}\coordHE{} with respect to the \myHighlight{$x^1$}\coordHE{} direction and rotated in the 
\myHighlight{$x^3, x^4$}\coordHE{} plane at an angle \myHighlight{$\beta_2$}\coordHE{} with respect to the
\myHighlight{$x^3$}\coordHE{} direction. The angles are given by
\begin{equation}\coord{}\boxEquation{
\mbox{cot}~\beta_i = B_i + F_i = Q_i^{-1}, i = 1, 2.
}{
\mbox{cot}~\beta_i = B_i + F_i = Q_i^{-1}, i = 1, 2.
}{ecuacion}\coordE{}\end{equation}
In the same time the direction \myHighlight{$x^2$}\coordHE{} is rotated by an angle
\myHighlight{$\pi/2 - \alpha_1$}\coordHE{} where \myHighlight{$\mbox{cot}~\alpha_1 = Q_1$}\coordHE{} 
and the direction \myHighlight{$x^4$}\coordHE{} gets rotated by an angle \myHighlight{$\pi/2 - \alpha_2$}\coordHE{}
where \myHighlight{$\mbox{cot}~\alpha_2 = Q_2$}\coordHE{}. 
...
A double reduction of the D9 brane in the \myHighlight{$x^2$}\coordHE{} direction gives a 
...
brane on \myHighlight{$(x'^2,\cdots,x^{10})$}\coordHE{} whose noncommutative 
...
difference is a factor \myHighlight{$\frac{\mbox{tan}~\alpha_1}{\mbox{tan}~\beta}$}\coordHE{}.
But this is exactly coming from the deformation of the torus and from the
rotation of the D-brane in the \myHighlight{$(x^1, x^2)$}\coordHE{} plane as explained above. 
So again there is a mapping between the 2-nd component of the BI vector and
the extra transverse direction.
The same discussion applies if we consider a T-duality on the \myHighlight{$x^4$}\coordHE{} direction.

If we take two T-dualities in the \myHighlight{$x^2, x^4$}\coordHE{} directions, we need to compared
...
make a double dimensional reduction on the \myHighlight{$x^4$}\coordHE{} direction, we obtain a 
...
component of the nine-dimensional BI vector in the direction \myHighlight{$x^4$}\coordHE{} over which
we reduce.
We then consider a direct dimensional reduction of a commutative 
D7 brane on the \myHighlight{$(x'^{2}, x'^{4},\cdots,x^{10})$}\coordHE{} directions from ten to eight 
...
where \myHighlight{$\tau' = \Phi^{4}$}\coordHE{} comes from the reduction on the transverse direction.
The supplementary factor \myHighlight{$\prod_{i=1}^{2}~\frac{Q_i}{\theta_i}$}\coordHE{} 
in (\ref{dr2}) comes from the rotation of the two tori \myHighlight{$T^2$}\coordHE{} and from 
the rotation of the directions of the D7 brane in the \myHighlight{$(x^1,x^2)$}\coordHE{} plane and
in the \myHighlight{$(x^3, x^4)$}\coordHE{} plane, therefore we see that the two components
\myHighlight{$\chi, \tau$}\coordHE{} of the BI vector are mapped into the extra transverse directions
\myHighlight{$\chi', \tau'$}\coordHE{}, as required by T duality.
...
If we consider the case of \myHighlight{$N$}\coordHE{} D9 branes on 
\myHighlight{$T^2$}\coordHE{} in the \myHighlight{$x^1, x^2$}\coordHE{} directions with magnetic fluxes 
\myHighlight{$F_{i},~i = 1,\cdots,N$}\coordHE{}, a T-duality in the \myHighlight{$x^1$}\coordHE{} direction would 
give a deformed \myHighlight{$T^2$}\coordHE{} torus by an angle \myHighlight{$\mbox{cot}~\alpha = B$}\coordHE{} and 
\myHighlight{$N$}\coordHE{} D8 branes rotated by angles 
\myHighlight{$\mbox{cot}~\beta_i = B + F_i,i = 1,\cdots,N$}\coordHE{}.
...
unstable and behave as D \myHighlight{$- \bar{\mbox{D}}$}\coordHE{} systems.
...
seen as a product of three tori \myHighlight{$T^2 \times T^2 \times T^2$}\coordHE{} 
in the \myHighlight{$(x^1, x^2)$}\coordHE{},
\myHighlight{$(x^3, x^4)$}\coordHE{} and \myHighlight{$(x^5,x^6)$}\coordHE{} directions respectively. 
We turn on \myHighlight{$B$}\coordHE{} and \myHighlight{$F$}\coordHE{} fields such that 
 \myHighlight{$B_{12} = B_1,~B_{34} = B_2,~B_{56} = B_3$}\coordHE{} and
 \myHighlight{$F_{12} = F_1,~F_{34} = F_2,~F_{56} = F_3$}\coordHE{} . We then have three values for
\myHighlight{$Q_i$}\coordHE{} and three values for \myHighlight{$\theta_i$}\coordHE{} and \myHighlight{$\mbox{Pf}~Q = Q_1~Q_2~Q_3$}\coordHE{} and
\myHighlight{$\mbox{Pf}~\theta = \theta_1~\theta_2~\theta_3$}\coordHE{}. 

By applying three T-dualities with respect to \myHighlight{$x^2, x^4, x^6$}\coordHE{}, the factor 
\myHighlight{$\frac{\mbox{Pf}~Q}{\mbox{Pf}~\theta}$}\coordHE{} appears naturally when comparing the
double and direct direction and the formula (\ref{cs1}) stands also
for this case.

Another check for the formulas of \cite{ms} is made when 
we start with a D7 brane on \myHighlight{$(x^1,\cdots,x^8)$}\coordHE{} directions with
\myHighlight{$B$}\coordHE{} and \myHighlight{$F$}\coordHE{} fields on the \myHighlight{$(x^1, x^2)$}\coordHE{} directions. As discussed in \cite{ms},
we have the transverse coordinates \myHighlight{$\Phi^8, \Phi^9$}\coordHE{} which are functions of the
...
\myHighlight{$Q^{12} = (F^{12} + B^{12})^{-1}$}\coordHE{}. 
The exponential term \myHighlight{$e^{i(i_{\Phi} * i_{\Phi})}$}\coordHE{}
just implies the appearance of a Myers-type term like 
\myHighlight{$Q^{9,10} = - i~[\Phi^{9}, \Phi^{10}]$}\coordHE{}. 

If we use the above discussion, a T-duality on the \myHighlight{$x^1$}\coordHE{} direction would give
...
Myers-type term because \myHighlight{$[\Phi^{9}, \Phi^{10}] = 0$}\coordHE{}, and this is expected
because the Chern-Simons term for the D6 brane involves only angles of
rotation which are related to \myHighlight{$Q^{12}$}\coordHE{} and \myHighlight{$\theta^{12}$}\coordHE{} and not to 
\myHighlight{$Q^{9,10}$}\coordHE{}.
...
where \myHighlight{${\cal D}_i T = -i~(Q^{-1})_{ij}~[X^j,T]$}\coordHE{} is a covariant derivative 
...
D-branes are not wrapped on tori but on \myHighlight{$S^2$}\coordHE{} cycles.
...
\myHighlight{$B$}\coordHE{} and \myHighlight{$F$}\coordHE{} fields on their world-volume and this implies  
Chern-Simons couplings which induce fractional D3 brane charges
\cite{do1,do2,kar,gi,dm2,k1,ot3,bere,ot4}.   
In the case of non-BPS systems of branes, by turning on different \myHighlight{$F$}\coordHE{} fluxes on
...
Walls in an \myHighlight{$N =1$}\coordHE{} Superconformal Gauge Theory},
...
{\it  Marginal and Relevant Deformations of \myHighlight{$N=4$}\coordHE{} 
