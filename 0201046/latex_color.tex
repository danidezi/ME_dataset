...
multiplicatively beyond \myHighlight{$d=4$}\coordHE{}. For the cases of three and six dimensions the
...
equations for any even \myHighlight{$d$}\coordHE{} as well as the counterterms needed to compensate
all the divergences. In the particular case of \myHighlight{$d=6$}\coordHE{} the results of our
...
force is derived in \myHighlight{$d=3$}\coordHE{}. It was also argued that d=3,4 are the only
...
the form of \myHighlight{$3d$}\coordHE{} integro-differential LD equation proposed in
...
restrict ourselves by considering a simple example of \myHighlight{$2+1$}\coordHE{} particle. The
...
So, let \myHighlight{${\mathbb{R}}^{d-1,1}$}\coordHE{} be \myHighlight{$d$}\coordHE{}-dimensional Minkowski space with
coordinates \myHighlight{$x^\mu $}\coordHE{}, \myHighlight{$\mu =0,...,d-1$}\coordHE{}, and signature \myHighlight{$(+-\cdots -)$}\coordHE{}.
Consider a scalar point particle of mass \myHighlight{$m$}\coordHE{} and charge \myHighlight{$e$}\coordHE{} coupled to
...
where \myHighlight{$F_{\mu \nu }=\partial _\mu A_\nu -\partial _\nu A_\mu $}\coordHE{} is strength
tensor of electromagnetic field \myHighlight{$A_\mu $}\coordHE{} and dot means the derivative with
respect to the particle's proper time \myHighlight{$\tau $}\coordHE{}\footnote{%
Strictly speaking, the parameter \myHighlight{$m$}\coordHE{} entering to the Lagrangian is, so
...
natural system of units (\myHighlight{$c=1,\hbar =1$}\coordHE{}) so that \myHighlight{$[m]=-[x]=1$}\coordHE{} and \myHighlight{$%
[e]=1-[A]=(4-d)/2$}\coordHE{}.
...
electromagnetic field produced by the moving charge. In the Lorenz gauge \myHighlight{$%
\partial ^\mu A_\mu =0$}\coordHE{} the equations take form
\begin{equation}\coord{}\boxEquation{
...
the point particle moving along the world-line \myHighlight{$x^\mu (\tau )$}\coordHE{} and the
Lorentz force:
\begin{equation}\coord{}\boxEquation{
...
whose repeat action \myHighlight{$D^nx^\mu (\tau )$}\coordHE{} on a trajectory remains intact under
...
depends strongly on the space-time dimension \myHighlight{$d$}\coordHE{}, and especially on its
parity. Mathematically, this manifests in quite different expressions for
the {\it retarded} Green function \myHighlight{$G=\square ^{-1}$}\coordHE{} associated to the
D'Alambert operator:
\begin{equation}\coord{}\boxEquation{
G(x)=\left\{
\begin{array}{ll}
...
where \myHighlight{${\bf x}=(x^1,\ldots x^{d-1})$}\coordHE{}. In an even-dimensional space-time the
...
Suppose a source of light is turned on at an initial time \myHighlight{$t$}\coordHE{} and is turned
then off at a time \myHighlight{$t^{\prime }$}\coordHE{}. If the number of space-time dimensions is
...
by the time interval \myHighlight{$t-t^{\prime }$}\coordHE{}. In so doing, the magnitude of the
...
by a finite-order differential equation for even \myHighlight{$d$}\coordHE{}'s and by an
...
\myHighlight{${\cal F}^{LD}$}\coordHE{} is the divergence of the integral for retarded field \myHighlight{$(\ref
...
of mass parameter \myHighlight{$m$}\coordHE{} only but brings about new renormalization constants
...
retarded Green function \myHighlight{$G$}\coordHE{}. The Green function (\myHighlight{$\ref{gf}$}\coordHE{}) being a kernel
of the inverse wave-operator (\ref{ret}) is well-defined when acting on a
smooth compactly supported source functions \myHighlight{$j_\mu(x)$}\coordHE{}. The difficulties may
...
for the case of \myHighlight{$d=3$}\coordHE{}, the Green functions involve products of generalized
...
be generalized functions of one variable, say \myHighlight{$x^0$}\coordHE{}, considering the other
variables as parameters. It is the problem we will face with by restricting
the argument of Green function onto particle's world-line \myHighlight{$x^{\mu}(\tau)$}\coordHE{}.
...
which is not differentiable at \myHighlight{$x^\mu =0$}\coordHE{}. As a result, \myHighlight{$G(x)$}\coordHE{} is singular
...
function \myHighlight{$F(s)=\delta(s^2)$}\coordHE{} defined on the real half-line \myHighlight{$s\geq 0$}\coordHE{}
\footnote{%
...
on the right.}. Relations \myHighlight{$s^2=0$}\coordHE{}, \myHighlight{$s>0$}\coordHE{} are the one-dimensional analog of (\myHighlight{$%
\ref{lc}$}\coordHE{}). We put by definition
\begin{equation}\coord{}\boxEquation{  \label{def}
\delta[f]=\int_0^{\infty}\delta(s)f(s)ds=f(0)
}{  \delta[f]=\int_0^{\infty}\delta(s)f(s)ds=f(0)
}{ecuacion}\coordE{}\end{equation}
Then the integral associated to linear functional \myHighlight{$F$}\coordHE{} reads
...
where \myHighlight{$f(x)$}\coordHE{} is a test function. In general, this integral diverges, so that
functional \myHighlight{$F$}\coordHE{} is ill-defined. Note, however, that integral (\ref{lf}) does
have meaning when evaluating on basic functions vanishing at zero. The
question is how to extend the functional \myHighlight{$F$}\coordHE{}, in a consistent way, from
...
function \myHighlight{$F(s)$}\coordHE{} and denote it by \myHighlight{$\text{reg} F(s)$}\coordHE{}. For example, the
...
\myHighlight{$a_0$}\coordHE{} being arbitrary constant. Indeed, the functional (\myHighlight{$\ref{reg}$}\coordHE{}) is
well-defined for any basic function \myHighlight{$f(x)$}\coordHE{} and coincides with (\myHighlight{$\ref{lf}$}\coordHE{})
if \myHighlight{$f(0)=0$}\coordHE{}. In fact, (\myHighlight{$\ref{reg}$}\coordHE{}) is the general solution to our problem
since the complementary space to that of vanishing-at-zero functions (i.e.
subspace on which \myHighlight{$F$}\coordHE{} comes to infinity) is one dimensional and spanned, for
example, by constant function \myHighlight{$f(s)=1$}\coordHE{}. Regularizing linear functional \myHighlight{$F$}\coordHE{}
we just replace the infinite value \myHighlight{$F[1]=\infty$}\coordHE{} by any finite constant \myHighlight{$%
\text{reg}F[1]=a_0$}\coordHE{}.
...
functions which are \myHighlight{$o(s^{2n+1})$}\coordHE{} as \myHighlight{$s\rightarrow +0$}\coordHE{}. Subtracting from a
function \myHighlight{$f$}\coordHE{} several first terms of its Taylor expansion we just project \myHighlight{$f$}\coordHE{}
onto the subspace of such functions. The value of functional on the
complementary (\myHighlight{$2n+1$}\coordHE{})-dimensional subspace is fixed by the arbitrary chosen
constants \myHighlight{$a_0, \ldots, a_{2n}$}\coordHE{}.
...
For our purposes it is instructive to re-derive expression \myHighlight{$(\ref{reg2})$}\coordHE{} in
another, perhaps more familiar for physicists, way. Namely, using the
sequential approach to generalized functions one may represent \myHighlight{$\delta$}\coordHE{}%
-function \myHighlight{$(\ref{def})$}\coordHE{} as
...
family of well-defined functionals until \myHighlight{$a\neq 0$}\coordHE{}:
\begin{equation}\coord{}\boxEquation{  \label{da}
F_a[f]=\int_0^{\infty}\frac{e^{-\frac{s^2}{a}}}{a}f(s)ds
}{  F_a[f]=\int_0^{\infty}\frac{e^{-\frac{s^2}{a}}}{a}f(s)ds
}{ecuacion}\coordE{}\end{equation}
In so doing, \myHighlight{$F[f]=\lim_{a\rightarrow 0}F_a[f]$}\coordHE{}. For this reason we refer to
\myHighlight{$a$}\coordHE{} as the regularization parameter. Given the basic function \myHighlight{$f(s)$}\coordHE{}, the
integral (\ref{da}) defines a meromorphic function in \myHighlight{$\sqrt{a}$}\coordHE{} given by
the Laurent series
...
In the limit \myHighlight{$a\rightarrow 0$}\coordHE{} of switching regularization off the
singularity of functional \myHighlight{$F[f]$}\coordHE{} appears as a simple pole, while the
non-vanishing term of the regular part of \myHighlight{$(\ref{ls})$}\coordHE{} coincides with (\ref
{reg}) if take \myHighlight{$a_0=0$}\coordHE{}. Similarly, the delta-shaped sequence (\ref{seq})
defines a holomorphic function in \myHighlight{$a$}\coordHE{} with coefficients being the
generalized functions in \myHighlight{$s$}\coordHE{},
...
We see that the regularization (\ref{reg2}) of functionals \myHighlight{$F^n$}\coordHE{} corresponds
to replacement
...
Now we are going to explicitly calculate the Lorentz-Dirac force \myHighlight{${\cal F}%
^{LD}$}\coordHE{}. As the results drastically differ in odd and even dimensions,
we consider these cases separately.

\subsection{ Even-dimensional space-time.}

The retarded Green functions are given by the first line in (\ref{gf}). We
start with the case of \myHighlight{$d=4$}\coordHE{}. The expression for LD force
...
particle, equation \myHighlight{$(x(s)-x(\tau ))^2=0$}\coordHE{} means \myHighlight{$x^\mu (s)=x^\mu (\tau )$}\coordHE{}
that in turn implies \myHighlight{$s=\tau $}\coordHE{} the integrand is supported at the point \myHighlight{$%
s=\tau $}\coordHE{}. Changing the integration variable \myHighlight{$\tau \rightarrow s-\tau $}\coordHE{} we
...
expansion for \myHighlight{${\cal F}_\mu ^{LD}(s,a)$}\coordHE{} in half-integer powers of \myHighlight{$a$}\coordHE{}. The
...
proper time \myHighlight{$\tau $}\coordHE{} to satisfy the additional normalization condition \myHighlight{$\dot
x^\mu \dot x_\mu =1$}\coordHE{}. In this gauge, many of terms vanish in the Taylor
...
of the regularized LD force is transverse to the particle's velocity, i.e. \myHighlight{$%
\dot x^\mu {\cal F}_\mu (s,a)=0$}\coordHE{}. Upon removing the regularization only the
first two terms of the series (\ref{ldf}) survive - one singular and one
finite - that leads to the well-known LD equation for a \myHighlight{$d=4$}\coordHE{} point charge
...
at \myHighlight{$s<0$}\coordHE{}. The different sequences will then lead to the different results
distinguished from each other by an overall constant factor. For example,
any symmetric (with respect to zero) approximation for \myHighlight{$\delta (s)$}\coordHE{} will
give the additional \myHighlight{$1/2$}\coordHE{} multiplier in the r.h.s. of rel. (\ref{def}).
...
unobservable bare mass \myHighlight{$m$}\coordHE{} and the infinite contribution due to the
...
so that renormalized action of \myHighlight{$d=4$}\coordHE{} particle takes the form
...
the desired
expressions for the LD force can be derived for any even \myHighlight{$d$}\coordHE{}
by successive differentiation of the universal series (\ref{ldf})
obtained for \myHighlight{$d=4$}\coordHE{}. More precisely,
\begin{equation}\coord{}\boxEquation{
...
derivatives of \myHighlight{$x$}\coordHE{}'s cannot be represented as the variation of a
Lorentz-invariant functional. These results generally agree with the
previous analysis of work \cite{Kos}, where the explicit expressions for \myHighlight{$6d$}\coordHE{}
...
normalization of \myHighlight{$6d$}\coordHE{} Li\'enard-Wiechert potentials accepted in \cite{Kos}.}.

This situation is general and nothing changes this picture as the number of
dimensions increases. Namely, for any even \myHighlight{$d$}\coordHE{} the finite part of the LD
force is given by a polynomial function in the derivatives of \myHighlight{$x^\mu (\tau )$}\coordHE{}
up to \myHighlight{$(d-1)$}\coordHE{} order inclusive. Note that the higher (odd) derivative \myHighlight{$%
D^{d-1}x^\mu $}\coordHE{} enters linearly to LD force and therefore no variation
...
there are \myHighlight{$d/2-1$}\coordHE{} divergent terms ''the most singular of which'' corresponds
...
not hard to check that in the cases of \myHighlight{$d=4,6$}\coordHE{} the regularized self-action
...
To summarize, for even dimension \myHighlight{$d>4$}\coordHE{} one force to extend the original
Lagrangian of the free relativistic particle by the addition of \myHighlight{$d/2-2$}\coordHE{}
...
mass, \myHighlight{$d/2-1$}\coordHE{} arbitrary constants.
...
of Green function entering to the strength tensor \myHighlight{$F_{\mu \nu }$}\coordHE{} of adjunct
...
of \myHighlight{$2+1$}\coordHE{} particle to illustrate how this technique works in the most simple
case.
...
In near the coincidence limit \myHighlight{$s\rightarrow \tau $}\coordHE{} the numerator and
denominator of the integrand (\ref{nl}) behave like \myHighlight{$(s-\tau)^2$}\coordHE{} and \myHighlight{$(s-\tau
)^3$}\coordHE{}, respectively, so that the integral diverges logarithmically, as one
...
nonlocal character of the self-action force in \myHighlight{$2+1$}\coordHE{} dimensions, the
...
In any even \myHighlight{$d$}\coordHE{}
the radiation reaction is given by the general formula (\ref{Fd})
implying only formal differentiation of \myHighlight{$4d$}\coordHE{} LD force by
regularization parameter.  It has been shown that contrary to the
quantum electrodynamics in \myHighlight{$d>4$}\coordHE{}, the classical electrodynamics of
point particles is a renormalizable theory, although
non-multiplicatively, beyond \myHighlight{$d=4$}\coordHE{}. The necessity of extra
...
relativistic particle. The models with \myHighlight{$N$}\coordHE{}-extended world-line supersymmetry
...
spinning particles of spin \myHighlight{$N/2$}\coordHE{}. By analogy with the quantum field theory
...
known, its back-reaction remains an open question even in \myHighlight{$d=4$}\coordHE{}.

The second option is to extend the above analysis to p-brain system
universally coupled to a \myHighlight{$(p+1)$}\coordHE{}-form field and other background fields. In this
...
of constraints on masses and couplings was established for \myHighlight{$0$}\coordHE{}-brain
