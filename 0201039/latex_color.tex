...
with zero spatial curvature and \myHighlight{$H \equiv {\dot a \over a} $}\coordHE{} the
...
Therefore, reversal from contraction (\myHighlight{$H<0$}\coordHE{}) to expansion (\myHighlight{$H>0$}\coordHE{})
is impossible.  (Here we assume that the time coordinate \myHighlight{$t$}\coordHE{}
covers the whole space.) We note that this statement is similar
to the proof of the \myHighlight{$c$}\coordHE{} theorem in the context of the AdS/CFT
correspondence.  There it is also shown that \myHighlight{$c$}\coordHE{} is monotonic.
...
We will consider the theory based on this Lagrangian in \myHighlight{$d$}\coordHE{}
spacetime dimensions.

In the minisuperspace description there are three degrees of
freedom \myHighlight{$a(\eta)$}\coordHE{}, \myHighlight{$N(\eta)$}\coordHE{} and \myHighlight{$\phi(\eta)$}\coordHE{} defined through
\begin{eqnarray}\coord{}\boxAlignEqnarray{\leftCoord{}
...
which are constrained to satisfy  \myHighlight{$a_0 > |a_1|$}\coordHE{}.  In terms of
...
\myHighlight{$\eta$}\coordHE{} with an appropriate transformation of \myHighlight{$N(\eta)$}\coordHE{}.  We can
choose a gauge \myHighlight{$N(\eta)=1$}\coordHE{} and impose the equation of motion of
\myHighlight{$N$}\coordHE{}; i.e.\  \myHighlight{$ \left( { da_0 \over d\eta}\right)^2 = \left( { da_1
\over d\eta}\right)^2$}\coordHE{} as a constraint.  This constraint
eliminates the negative mode associated with \myHighlight{$a_0$}\coordHE{}.  (This
negative mode is common in gravity and is familiar in the
worldsheet description of string theory.)  The coordinates \myHighlight{$a_0$}\coordHE{}
and \myHighlight{$a_1$}\coordHE{} are free except that they are bounded: \myHighlight{$a_0 > |a_1|$}\coordHE{}.
...
\myHighlight{$a(1)$}\coordHE{} and \myHighlight{$\phi(1)$}\coordHE{} are two integration constants. We will focus
...
singularity with \myHighlight{$a=0$}\coordHE{} and \myHighlight{$\phi=-\infty$}\coordHE{} which happens at
\myHighlight{$\eta=0$}\coordHE{}.  The other solution which exists for positive \myHighlight{$\eta$}\coordHE{}
represents expansion from a singularity with \myHighlight{$a=0$}\coordHE{} and
\myHighlight{$\phi=-\infty$}\coordHE{} which takes place at \myHighlight{$\eta=0$}\coordHE{}.  In terms of the
coordinates \myHighlight{$a_0$}\coordHE{} and \myHighlight{$a_1$}\coordHE{} the motion is free; i.e.\ linear in
\myHighlight{$\eta$}\coordHE{}.  These variables are finite at \myHighlight{$\eta=0$}\coordHE{}, where they reach
their bound \myHighlight{$a_0=a_1$}\coordHE{}.

It is natural to conjecture that the two solutions can be
connected at the singularity at \myHighlight{$\eta=0$}\coordHE{}.  Then the free
classical motion of \myHighlight{$a_0$}\coordHE{} and \myHighlight{$a_1$}\coordHE{} simply bounces off the bound
\myHighlight{$a_0=a_1$}\coordHE{}, such that both for \myHighlight{$\eta$}\coordHE{} negative and for \myHighlight{$\eta$}\coordHE{}
positive \myHighlight{$a_0 > a_1$}\coordHE{}.  We thus conjecture that our system
...
We see that the metric \myHighlight{$\bar g_{\mu\nu}$}\coordHE{} is independent of
\myHighlight{$\eta$}\coordHE{}.  In particular, it is smooth at the transition point
\myHighlight{$\eta=0$}\coordHE{}, it is nonsingular and does not exhibit reversal from
contraction to expansion.  The solution of \myHighlight{$\psi$}\coordHE{} is singular at
\myHighlight{$\eta=0$}\coordHE{} and  since \myHighlight{$\psi(0)=0$}\coordHE{}, the Planck scale goes to zero
there.  In these variables the singularity does not appear as a
short distance singularity (because \myHighlight{$\bar g_{\mu\nu}$}\coordHE{} is finite)
...
This theory can originate from the compactification of a \myHighlight{$d+1$}\coordHE{}
...
direction.  More explicitly, let the metric of the \myHighlight{$d+1$}\coordHE{}
...
modes (\myHighlight{$w$}\coordHE{} dependence in \myHighlight{$\psi$}\coordHE{} and \myHighlight{$\bar g_{\mu\nu}$}\coordHE{}).  Then,
the action (\ref{dpoac}) is simply the standard Hilbert-Einstein
action in \myHighlight{$d+1$}\coordHE{} dimensions of the metric
\myHighlight{$g^{(d+1)}_{\alpha\beta}$}\coordHE{} of (\ref{dpom})
\begin{eqnarray}\coord{}\boxAlignEqnarray{\leftCoord{}
...
In terms of the metric \myHighlight{$g^{(d+1)}$}\coordHE{} up to rescaling the
...
with a constant \myHighlight{$A$}\coordHE{}.  We see here a flat \myHighlight{$d-1$}\coordHE{} dimensional space
times a two dimensional space \myHighlight{${\cal M}^2$}\coordHE{}.

The metric of \myHighlight{${\cal M}^2$}\coordHE{} is
\begin{eqnarray}\coord{}\boxAlignEqnarray{\leftCoord{}
...
The expression in terms of \myHighlight{$x^\pm$}\coordHE{} shows that the space is
locally flat.  But the identification \myHighlight{$w\sim w+1$}\coordHE{} identified
\myHighlight{$x^\pm \sim e^{\pm A} x^\pm $}\coordHE{}; i.e.\ the identification is by a
...
space (a quadrant of the whole \myHighlight{$x^\pm$}\coordHE{} space).  With the
identification by a boost the space \myHighlight{${\cal M}^2$}\coordHE{} is more subtle.
...
\myHighlight{$-\infty <t < \infty$}\coordHE{}; i.e.\ time flows from minus infinity to
...
\myHighlight{$a$}\coordHE{}, and hence it takes only a finite time.
...
The conjectured two cone geometry \myHighlight{${\cal M}^2$}\coordHE{} can be embedded in
string theory in many ways.  Let us mention a few of them:

\begin{itemize}
\item
M theory on \myHighlight{$R^9\times {\cal M}^2$}\coordHE{}.  This background is flat
except at \myHighlight{$t=0$}\coordHE{}.  Therefore it is an interesting candidate to be a
solution of the equations of motion.  Since spacetime
supersymmetry is broken, it is not obvious that \myHighlight{$R^9\times {\cal
M}^2$}\coordHE{} is an exact solution of the full equations of motion of M
theory.  However, for large \myHighlight{$|t|$}\coordHE{} the background is approximately
flat \myHighlight{$R^{10,1}$}\coordHE{} compactified on a large circle. Since the circle
...
for large \myHighlight{$|t|$}\coordHE{} this background is an approximate solution of the
equations.

We can view this background as a time dependent background for
type IIA string theory on \myHighlight{$R^9\times R$}\coordHE{} where \myHighlight{$R$}\coordHE{} is the time
coordinate in \myHighlight{${\cal M}^2$}\coordHE{}. This description is weakly coupled
and has small curvature for a certain range of \myHighlight{$t$}\coordHE{} near \myHighlight{$t\approx
0$}\coordHE{} (but not too close to zero) and for an appropriate range of the
parameter \myHighlight{$A$}\coordHE{}.  There it is straightforward to see that the
leading order equations of motion of type IIA string theory are
satisfied.

\item
M theory on \myHighlight{$R^9\times {\cal M}^2/Z_2$}\coordHE{}.  This is a \myHighlight{$Z_2$}\coordHE{} orbifold
...
each other at negative \myHighlight{$t$}\coordHE{}, collide at \myHighlight{$t=0$}\coordHE{} and bounce off each
other at positive \myHighlight{$t$}\coordHE{}.  The entire process looks like two banging
cymbals.

\item
M theory on \myHighlight{$R^8\times S^1\times {\cal M}^2$}\coordHE{}.  This is an \myHighlight{$S^1$}\coordHE{}
compactification of the first example and can be interpreted as
type IIA string theory on \myHighlight{$R^8\times {\cal M}^2$}\coordHE{}.  Here the type
IIA background is flat except at \myHighlight{$t=0$}\coordHE{}, where the curvature is
