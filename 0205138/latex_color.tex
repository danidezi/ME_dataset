...
which preserve \myHighlight{$1/4$}\coordHE{} of the full type IIA supersymmetry. 
These membrane configurations carry additional F-string charges.
We further analyze the \myHighlight{$D1-D3$}\coordHE{} configuration after applying T- duality 
...
by now that a system of multiple D- branes related by \myHighlight{$SU(N)$}\coordHE{} 
...
are parametrized by a group \myHighlight{$O(1,1; 1,1)$}\coordHE{}. In this case, the complete solution 
...
\cite{myers1,hambli}. It preserves \myHighlight{$1/4$}\coordHE{} of the full IIA supersymmetry.
We then apply \myHighlight{$T$}\coordHE{}- duality along one of the world volume directions to
generate bound state configuration of orthogonal (D1, D3) branes.
The existence and stability of \myHighlight{$(D1 \bot D3)$}\coordHE{} is checked by
...
charge neutral D2-brane solution\cite{kastor,duff} by taking \myHighlight{$\alpha = 0$}\coordHE{}
limit.  
...
where \myHighlight{$X = C ({l \over{|\vec{x}|}})^{5}$}\coordHE{} is the harmonic function in
the transverse space and \myHighlight{$l$}\coordHE{} is the length parameter.
...
in \myHighlight{$x^8-x^9$}\coordHE{} plane and are parallel to each other. 
A rotation between \myHighlight{$x^8-x^9$}\coordHE{} and \myHighlight{$x^1-x^2$}\coordHE{}
plane is then performed following \cite{myers1}. 
The rotation angles in our case 
are \myHighlight{$(\theta_1, \theta_2) = (0, \pi/2)$}\coordHE{}, where \myHighlight{$\theta_1$}\coordHE{} and
\myHighlight{$\theta_2$}\coordHE{} are the angles with which the two membranes are being
rotated. The second rotated brane now lies in \myHighlight{$x^1-x^2$}\coordHE{} plane which
is orthogonal to the \myHighlight{$x^8-x^9$}\coordHE{} plane. The final solution is given by:  
\begin{eqnarray}\coord{}\boxAlignEqnarray{\leftCoord{}
ds^2 &=& {(1 + X)^{3/2}\leftCoord{}\over\rightCoord{} {1 + X \cosh^2 \alpha}}\Big[{1\leftCoord{}\over\rightCoord{} {1 + \rightCoord{}
...
where \myHighlight{$X_{a} = C ({l_a \over{|\vec{x} - \vec{x_{a}}|}})^{3}$}\coordHE{},
with \myHighlight{$a = 1, 2$}\coordHE{}, are the harmonic functions in the tranverse space,
given by \myHighlight{$x^i$}\coordHE{}'s, and \myHighlight{$ X = ( X_1 + X_2 + X_1 X_2)$}\coordHE{}\cite{myers1,hambli} with
\myHighlight{$l_a$}\coordHE{}'s being the arbitrary
positive parameters having the dimensions of length. 
To clarify further the form of \myHighlight{$X$}\coordHE{} appearing above, one notices that
...
with \myHighlight{$\theta_1$}\coordHE{} and \myHighlight{$\theta_2$}\coordHE{} are the angles with which the branes
being rotated. In the present context, however, 
(\myHighlight{$\theta_1, \theta_2) = (0, \pi/2)$}\coordHE{}. 
We would like to point out here, that for a configuration of
parallel membranes \myHighlight{$X$}\coordHE{} is the sum of individual 
harmonic functions describing each of them. But, 
when the branes are oriented at some angle, the form of \myHighlight{$X$}\coordHE{} gets
...
\myHighlight{$\alpha=0$}\coordHE{} limit of the above solution reduces 
...
3- form R-R potentials which are in \myHighlight{$(x^8-x^9)$}\coordHE{} and
\myHighlight{$(x^1-x^2)$}\coordHE{}-plane. In addition it
also carries \myHighlight{$F$}\coordHE{}-string charges along \myHighlight{$x^1$}\coordHE{} and \myHighlight{$x^9$}\coordHE{} directions.  
...
\myHighlight{$A^1_t \sim {\cal{B}}_{t 9}$}\coordHE{}, 
\myHighlight{$A^2_t \sim {\cal{B}}_{t 1}$}\coordHE{}  \myHighlight{$A^3_t \sim {\cal{A}}_{t 8 9}$}\coordHE{} and \myHighlight{$A^4_t \sim {\cal{A}}_{t 1 2}$}\coordHE{},
...
where, \myHighlight{$Q_i$}\coordHE{}'s are the charges associated with \myHighlight{$A_t^{(i)}$}\coordHE{} with \myHighlight{$i=
1....4$}\coordHE{}. 
...
where \myHighlight{$n^i$}\coordHE{} is a radial unit vector in the transverse space and 
\myHighlight{$h_{\mu \nu}$}\coordHE{} is the deformation of the Einstein-frame metric
...
where \myHighlight{$q_{i j}$}\coordHE{}'s are the nonvanishing charge densities
correspond to D- membranes.
By suitably identifying the charges in our case, with \myHighlight{$q_{i j}$}\coordHE{}'s 
of \cite{myers1}, eqn.(\ref{mass-charge}) does imply the mass-formula
of one-quarter BPS objects. We therefore conclude that our
configuration preserves \myHighlight{$1/4$}\coordHE{} supersymmetry.
...
direction, namely along \myHighlight{$x^9$}\coordHE{}. Following the rules, that are given in 
...
Moreover, setting \myHighlight{$X_1 = 0$}\coordHE{}, one can verify that the solution
...
\myHighlight{$A^1_t \sim {\cal{A}}_{t 8}$}\coordHE{}, 
\myHighlight{$A^2_t \sim {\cal{B}}_{t 1}$}\coordHE{},
\myHighlight{$A^3_t \sim {\cal{A}}_{t 1 2 9}$}\coordHE{}, 
\myHighlight{$A^4_t \sim {\cal{A}}_{t 1 8 9}$}\coordHE{} and \myHighlight{$A^5_t \sim g_{t 9}/g_{9 9}$}\coordHE{}.
...
where \myHighlight{$Q_i$}\coordHE{}'s and \myHighlight{$P$}\coordHE{} are the charges corresponding to the gauge fields
\myHighlight{$A^i_t$}\coordHE{},(i= 1, 2, 3) and \myHighlight{$A^5_i$}\coordHE{} respectively.
\myHighlight{$P$}\coordHE{} is the momentum in the \myHighlight{$x^9$}\coordHE{} direction of the 10- dimensional
theory.
The mass density of \myHighlight{$(D1\bot D3)$}\coordHE{} is given by:
\begin{equation}\coord{}\boxEquation{
...
again indicates that a bound of \myHighlight{$1/4$}\coordHE{} BPS objects is saturated. 
