...
Consider a point particle whose trajectory is described by the coordinates \myHighlight{$%
x^{\mu}(\tau),$}\coordHE{} where \myHighlight{$\tau$}\coordHE{} is the proper time parameter. The geodesic
equation is
...
Here, \myHighlight{$\Gamma _{\alpha \beta }^{\mu }(x)$}\coordHE{} stands for the Christoffel symbols.
...
where \myHighlight{$\frac{D}{D\tau }$}\coordHE{} denotes covariant derivative with respect to \myHighlight{$\tau $}\coordHE{}
and \myHighlight{$\frac{D^{2}x^{\mu }}{D\tau ^{2}}=\frac{D}{D\tau }(\frac{Dx^{\mu }}{%
D\tau })$}\coordHE{}. Note that, since the coordinates \myHighlight{$x^{\mu }$}\coordHE{} are scalars fields,
one has \myHighlight{$\frac{Dx^{\mu }}{D\tau }=\frac{dx^{\mu }}{d\tau }.$}\coordHE{}

A nearby point particle must also satisfy the geodesic equation. If we use
the coordinates \myHighlight{$x^{\prime \mu }(\tau )$}\coordHE{} to describe the position of such a
...
By nearby we mean that the coordinates \myHighlight{$x^{\prime\mu}(\tau)$}\coordHE{} can be written
as
...
with \myHighlight{$\Gamma _{\alpha \beta }^{\mu },_{\lambda }=\frac{\partial \Gamma
_{\alpha \beta }^{\mu }}{\partial x^{\lambda }}$}\coordHE{}. Thus, using (4) and (5) we
...
where \myHighlight{$R_{\alpha \lambda \beta }^{\mu }$}\coordHE{} is the usual curvature Riemann
tensor,
...
Here \myHighlight{$S^{\mu \nu }=-S^{\nu \mu }$}\coordHE{} is the internal angular momentum (or the
spin tensor) per unit mass of the top satisfying the Pirani constraint [26] \myHighlight{$%
S^{\mu \nu }\frac{dx_{\nu }}{d\tau }=0$}\coordHE{}. It is worth mentioning that the
...
spinning top is described by the variables \myHighlight{$x^{\mu }(\tau )$}\coordHE{} and \myHighlight{$\theta
...
rotation of the top is described by the four vectors \myHighlight{$e_{(\alpha )}^{\mu
}(\tau ).$}\coordHE{} Specifically, in the case of RHGT the spin tensor \myHighlight{$S^{\mu \nu }$}\coordHE{}
is given by \myHighlight{$S^{\mu \nu }=i\theta ^{\mu }\theta ^{\nu }$}\coordHE{}, while in the
Hojman's approach \myHighlight{$S^{\mu \nu }$}\coordHE{} is the canonical momentum associated to the
angular velocity \myHighlight{$\sigma ^{\mu \nu }=e^{\mu (\alpha )}\frac{D}{D\tau }%
e_{(\alpha )}^{\nu }$}\coordHE{}. In both cases the corresponding Lagrangian is taken
...
Tulczyjew constraints [31] \myHighlight{$S^{\mu \nu }P_{\nu }=0,$}\coordHE{} with \myHighlight{$P_{\mu }$}\coordHE{} the
linear momentum of the top, rather than the Pirani constraint. As a result
the linear momentum \myHighlight{$P_{\mu }$}\coordHE{} turns out to be non parallel to the velocity \myHighlight{$%
u^{\mu }=\frac{dx^{\mu }}{d\tau }$}\coordHE{}. However this difference is very slight
...
reduces to (1) when the spin tensor \myHighlight{$S^{\mu \nu }$}\coordHE{} vanishes.
...
to first order in \myHighlight{$\xi ^{\mu }.$}\coordHE{} We find
...
where we have used the definition of the Riemann tensor \myHighlight{$R_{\alpha \lambda
\beta }^{\mu }.$}\coordHE{}
...
Here, for any contravariant tensor \myHighlight{$A^{\mu }$}\coordHE{} we define the covariant
derivative as \myHighlight{$A^{\mu };_{\sigma }=A^{\mu },_{\sigma }+\Gamma _{\sigma
\gamma }^{\mu }A^{\gamma }.$}\coordHE{} Although the result (27) seems to be evident,
...
(RTDE). Clearly, (28) reduces to (13) when the spin tensor \myHighlight{$S^{\lambda \beta
}$}\coordHE{} vanishes. Therefore, (28) is an extension of (13). (It is worth
mentioning that in Ref. [32] appears similar equations to those in (28).)
One of the attractive features of (28) is that the spin angular momentum \myHighlight{$%
S^{\lambda \beta }$}\coordHE{} of the top is coupled to gravity via the curvature
Riemann tensor \myHighlight{$R_{\alpha \lambda \beta }^{\mu }$}\coordHE{} and the gradient of this.
...
where \myHighlight{$\eta _{\mu \nu }$}\coordHE{} is the Minkowski metric and \myHighlight{$\mid h_{\mu \nu }\mid
\ll 1.$}\coordHE{} \ In the transverse-traceless gauge
...
with the indices \myHighlight{$i,j,...,etc$}\coordHE{} running from \myHighlight{$1$}\coordHE{} to \myHighlight{$3$}\coordHE{}. The Einstein
gravitational field equations imply that \myHighlight{$h_{ij}$}\coordHE{} satisfies the wave equation

\begin{equation}\coord{}\boxEquation{
\square ^{2}h_{ij}=0,  \label{31}
}{
\square ^{2}h_{ij}=0,  }{ecuacion}\coordE{}\end{equation}
where \myHighlight{$\square ^{2}=\partial ^{\mu }\partial _{\mu }$}\coordHE{} is the D'Alambertian.
In the gauge given in (30), the space-time components of the Riemann tensor \myHighlight{$%
R_{i0j0}$}\coordHE{} have the simple form
...
By considering (30), one discovers that \myHighlight{$h_{ij}$}\coordHE{} can be written as

\begin{equation}\coord{}\boxEquation{
h_{ij}=A_{+}e_{ij}^{+}+A_{\times }e_{ij}^{\times },  \label{33}
}{
h_{ij}=A_{+}e_{ij}^{+}+A_{\times }e_{ij}^{\times },  }{ecuacion}\coordE{}\end{equation}
where \myHighlight{$A_{+}$}\coordHE{} and \myHighlight{$A_{\times }$}\coordHE{} are two independent dimensionless amplitudes
and \myHighlight{$e_{ij}^{+}$}\coordHE{} and \myHighlight{$e_{ij}^{\times }$}\coordHE{} are polarization tensors. For a wave
traveling in the \myHighlight{$z-$}\coordHE{}direction the only nonvanishing components of \myHighlight{$e_{ij}$}\coordHE{}
are
...
and in this case \myHighlight{$A_{+}$}\coordHE{} and \myHighlight{$A_{\times }$}\coordHE{} turn out to be functions that
depend only on \myHighlight{$t-z$}\coordHE{}.

In a proper reference frame we have \myHighlight{$x^{0}=\tau ,$}\coordHE{}  \myHighlight{$x^{i}=0,$}\coordHE{} so that \myHighlight{$\frac{%
dx^{0}}{d\tau }=1$}\coordHE{} and \myHighlight{$\frac{dx^{i}}{d\tau }=0.$}\coordHE{} In this reference frame we
...
with \myHighlight{$h_{j}^{i}=\delta ^{ik}h_{kj}$}\coordHE{}, and for a wave propagating in the \myHighlight{$z-$}\coordHE{}%
...
where now, the latin indices \myHighlight{$a,b,..,etc$}\coordHE{} run from \myHighlight{$1$}\coordHE{} to \myHighlight{$2$}\coordHE{}. The formula
...
where we used the fact that \myHighlight{$S^{0\mu }=0$}\coordHE{} due to the Twlczyjew-Pirani
constraint \myHighlight{$S^{\mu \nu }\frac{dx_{\nu }}{d\tau }=0$}\coordHE{}.
...
plane wave propagating in the \myHighlight{$z-$}\coordHE{}direction. It is convenient to write the
indices \myHighlight{$i,j...etc$}\coordHE{} as (\myHighlight{$a,z).$}\coordHE{} With this notation (40) can be written as
...
The\myHighlight{$\ z$}\coordHE{} component of (44) is
...
where we used (43), and the \myHighlight{$x$}\coordHE{} and \myHighlight{$y$}\coordHE{} components are
...
Note that the \myHighlight{$S^{ab}$}\coordHE{} component of the spin angular momentum does not
appear in (42), (45) and (46) and that only the \myHighlight{$S^{zb}$}\coordHE{} component remains\myHighlight{$.$}\coordHE{}
...
where \myHighlight{$\varepsilon ^{ijk}$}\coordHE{} is the Levi-Civita tensor with \myHighlight{$\varepsilon
^{xyz}=1.$}\coordHE{} From (47) we see that the \myHighlight{$z$}\coordHE{} component of the intrinsic angular
...
The second interesting observation is that if \myHighlight{$S^{zb}$}\coordHE{} is nonvanishing then \myHighlight{$%
\frac{d^{2}\xi ^{0}}{dt^{2}}\neq 0$}\coordHE{} and \myHighlight{$\frac{d^{2}\xi ^{z}}{dt^{2}}\neq 0,$}\coordHE{}
in contrast with the case of a pair of nearby point particles in which both
of these terms vanish. The third important observation is that the \myHighlight{$\frac{%
d^{2}\xi ^{a}}{dt^{2}}$}\coordHE{} equation contains a large number of terms in
addition to the usual one \myHighlight{$R_{0b0}^{a}\xi ^{b}.$}\coordHE{} Clearly, this means that
the solution of (46) will not be as simple as in the case of nearby point
particles.

Let us focus on the terms in (46) not involving derivatives of \myHighlight{$S^{zb}$}\coordHE{} and
...
respectively. From (49) and (50) we discover that we can set \myHighlight{$\xi ^{0}=-\xi
^{z}+cte.$}\coordHE{} Therefore (51) becomes
...
Observe that if at an initial time the spin of the top \myHighlight{$S^{bz}$}\coordHE{} has an
...
then there is not transverse motion \myHighlight{$\xi^{a}=const.$}\coordHE{} and therefore (50)
admits the solution
...
separation \myHighlight{$\xi ^{a}$}\coordHE{} of the two tops, then the gravitational wave does not
...
where \myHighlight{$\xi _{0}^{a}=const$}\coordHE{}. We observe that \myHighlight{$\frac{d\xi ^{a}}{dt}=\frac{1}{2}%
h_{b}^{a},_{0}\xi _{0}^{b}-h_{b}^{a},_{0}(\frac{d\xi ^{z}}{dt}\mid
_{0})S^{bz}$}\coordHE{}. Substituting (55) into (50) we find that to first order in \myHighlight{$h$}\coordHE{}
(50) becomes
...
Therefore if initially \myHighlight{$\frac{d\xi ^{z}}{dt}\mid _{0}=0$}\coordHE{} to first order of
...
approximation in the perturbation \myHighlight{$h_{ab}$}\coordHE{}. At first sight, it could seem
...
Here, we are interested in making a rough estimate for \myHighlight{$\eta ^{a}$}\coordHE{}. The
result \myHighlight{$\xi ^{0}=-\xi ^{z}+const.$}\coordHE{} allows to set \myHighlight{$(\frac{d\xi ^{z}}{dt}\mid
_{0})\sim 1.$}\coordHE{} Thus, (58) is reduced to

\begin{equation}\coord{}\boxEquation{
\eta ^{a}\sim h_{b}^{a}\frac{S^{bz}}{m_{0}c},  \label{59}
}{
\eta ^{a}\sim h_{b}^{a}\frac{S^{bz}}{m_{0}c},  }{ecuacion}\coordE{}\end{equation}
where, in order to have the correct units, we restored the constants \myHighlight{$m_{0}$}\coordHE{}
and \myHighlight{$c.$}\coordHE{} (Here, \myHighlight{$m_{0}$}\coordHE{} is the mass of the top and \myHighlight{$c$}\coordHE{} denotes the light
...
Roughly we can write \myHighlight{$S\sim m_{0}r^{2}\omega $}\coordHE{}, so (60) becomes
...
For typical milli second (ms) pulsars \myHighlight{$\omega \sim 10^{3}Hz$}\coordHE{} and \myHighlight{$r\sim
10^{6}cm$}\coordHE{}. Therefore, \myHighlight{$\frac{r^{2}\omega }{c}\sim 10^{15}cm$}\coordHE{} and from (61)
we find that
...
If we consider the best expected sensitivities of the wave amplitude \myHighlight{$h$}\coordHE{}
over the earth, roughly \myHighlight{$h\sim 10^{-18}$}\coordHE{}, we find that \myHighlight{$\eta \sim
10^{-13}cm. $}\coordHE{} This value for \myHighlight{$\eta $}\coordHE{} is, of course, too small for current
research detections. However, this estimate is based on the value \myHighlight{$h\sim
10^{-18},$}\coordHE{} which corresponds to gravitational radiation sources at distances
...
where \myHighlight{$Q_{jk}$}\coordHE{} is the mass quadrupole moment, one finds that the expected
value for \myHighlight{$h$}\coordHE{} should be increased by several orders of magnitude. For
...
where \myHighlight{$r_{sch}$}\coordHE{} is the Schwarzschild radius and \myHighlight{$R$}\coordHE{} is the radius of the
source system, one finds that the value of \myHighlight{$h$}\coordHE{} could be, for instance, of
the order \myHighlight{$10^{-1}$}\coordHE{}, which leads to a sensitivity of \myHighlight{$\eta \sim 10^{4}cm.$}\coordHE{}
...
spin tensor \myHighlight{$S^{\lambda \beta }$}\coordHE{} is identified as the fermionic spin (\myHighlight{$%
S^{\mu \nu }=i\theta ^{\nu }\theta ^{\mu }$}\coordHE{}) of elementary particles.
