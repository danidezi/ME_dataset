...
where \myHighlight{$c$}\coordHE{} is the regularization parameter and we take \myHighlight{$c \rightarrow +
\infty $}\coordHE{} at the end.  This superpotential leads to the
...
supersymmetry is maintained in the limit \myHighlight{$c\rightarrow +\infty$}\coordHE{} in
...
\myHighlight{$\omega = \frac{1}{2}$}\coordHE{} for simplicity)
\begin{equation}\coord{}\boxEquation{
...
eigenstates. Gangopadhyaya and Mallow claim that \myHighlight{$\epsilon = 0$}\coordHE{} is not
a solution of the above equation, in which case 
supersymmetry will be broken. It is, of course, obvious that \myHighlight{$\epsilon
= 0$}\coordHE{} is a solution of (\ref{boundary}) for any value of \myHighlight{$c$}\coordHE{} and, in
particular, for \myHighlight{$c\rightarrow +\infty$}\coordHE{}, but let us look at this more
...
\myHighlight{$\epsilon=\epsilon_{n}$}\coordHE{}.   Let us, in
particular, investigate whether \myHighlight{$\epsilon = 0$}\coordHE{} is a solution of the
equation, and if it is, whether it remains a solution
in the limit \myHighlight{$c \rightarrow +\infty$}\coordHE{}.

Let us note that both \myHighlight{$F(\epsilon)$}\coordHE{} and \myHighlight{$G(\epsilon,c)$}\coordHE{}, as complex
functions  of \myHighlight{$\epsilon$}\coordHE{}, 
are analytic around \myHighlight{$\epsilon=0$}\coordHE{}.  For \myHighlight{$F(\epsilon)$}\coordHE{}, we can see this
from the fact
that the function \myHighlight{$\frac{1}{\Gamma (-\epsilon)}$}\coordHE{} is entire in the
whole \myHighlight{$\epsilon$}\coordHE{} 
plane with simple zeroes at the points \myHighlight{$\epsilon = n$}\coordHE{}, \myHighlight{$(n = 0,1,2,...)$}\coordHE{},
and the function \myHighlight{$\Gamma (\frac{1}{2} - \epsilon)$}\coordHE{} is also analytic around
\myHighlight{$\epsilon=0$}\coordHE{},
with \myHighlight{$\Gamma (\frac{1}{2}) = \sqrt{\pi}$}\coordHE{}. For \myHighlight{$G(\epsilon,c)=
\sqrt{c^2 - 2 \epsilon} -c $}\coordHE{}, the same is also true.

Since both these functions are analytic around \myHighlight{$\epsilon = 0$}\coordHE{}, a
...
The fact that \myHighlight{$\epsilon = 0$}\coordHE{} is a solution of equation
...
this  result is independent of \myHighlight{$c$}\coordHE{} and, therefore, also holds in the limit \myHighlight{$c
\rightarrow + \infty$}\coordHE{}.
...
the equation below equation (11) (for \myHighlight{$\omega = \frac{1}{2}$}\coordHE{}) in \cite{asim} is
...
values of 
\myHighlight{$\epsilon$}\coordHE{}, not even in a neighborhood of \myHighlight{$\epsilon=0$}\coordHE{}. It should 
hold only  for discrete values of
\myHighlight{$\epsilon$}\coordHE{}.  For the case \myHighlight{$\epsilon=0$}\coordHE{}, this translates into the fact
that only the
zeroth order terms in the Taylor expansion of \myHighlight{$F$}\coordHE{} and \myHighlight{$G$}\coordHE{} should
