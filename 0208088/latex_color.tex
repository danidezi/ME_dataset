...
\myHighlight{$m^2 A^2$}\coordHE{} appears in the initial Lagrangian. 
\end{abstract}
...
g \epsilon^{abc}A^b_\mu A^c_\nu$}\coordHE{} is the field strength and \myHighlight{$A^a_\mu$}\coordHE{} 
is the gauge potential; \myHighlight{$g$}\coordHE{} is the coupling constant; 
\myHighlight{$\epsilon^{abc}$}\coordHE{} are the structural constants of the  
gauge group SU(2); \myHighlight{$a=1,2,3$}\coordHE{}. In the quantum case we have the operators 
\myHighlight{$\hat A^a_\mu$}\coordHE{} and \myHighlight{$\hat F^a_{\mu \nu} = \partial_\mu \hat A^a_\nu - 
...
\myHighlight{$\hat F^a_{\mu \nu}$}\coordHE{} : 
\myHighlight{$(\hat F_{nl})^a_{\mu \nu} = \epsilon ^{abc} \hat A^b_\mu \hat A^c_\nu$}\coordHE{}. 
At first we assume 
...
\myHighlight{$[\hat A^a_\mu , \hat A^b_\nu]$}\coordHE{}. The second term in Eq. \eqref{sec1-30} 
after some manipulations with 
brackets and permutations of \myHighlight{$A^a_\mu$}\coordHE{} can be led to 
\myHighlight{$(\hat A^a_\mu \hat A^{a \mu})(\hat A^b_\nu \hat A^{b \nu})$}\coordHE{}. 
In this case this equation 
tell us that the square of nonlinear part \myHighlight{$(F_{nl})^a_{\mu \nu}$}\coordHE{} 
of the field strength is not equal to the square of the vector 
square \myHighlight{$\hat A^4 = (\hat A^a_\mu \hat A^{a\mu})^2$}\coordHE{}, 
...
the nonlinearity of the term \myHighlight{$(\hat F_{nl})^2$}\coordHE{}. In order to calculate 
this difference we will consider at first a very simple example. 

\section{Simple example}

Let \myHighlight{$\hat a$}\coordHE{} and \myHighlight{$\hat b$}\coordHE{} are operators which have a nonassociative property
\begin{equation}\coord{}\boxEquation{
...
where \myHighlight{$\hat \phi (x)$}\coordHE{} is some field operator and \myHighlight{$D(x - y)$}\coordHE{} is some function. 
We see that at the l.h.s. we have the production of two 
operators and at the r.h.s. the number of operators is 
\myHighlight{$(2-2)=0$}\coordHE{}. 
\par 
...
\myHighlight{$(4-2)=2$}\coordHE{} operators. Thus, we have two possibilities 
\begin{equation}\coord{}\boxEquation{
\left ( \hat a \hat b\right ) \left ( \hat a \hat b\right ) - 
\left ( \left ( \hat a \hat b \right ) \hat a \right ) \hat b = 
\lambda 
...
The next simple example is for the vectors \myHighlight{$A_\mu$}\coordHE{}. 
...
\eqref{sec3-90} 
\textit{are not connected with the commutator} \myHighlight{$[\hat a , \hat b]$}\coordHE{} 
which can be zero. This is only a manifestation of the nonlinearity 
of the corresponding expression 
\myHighlight{$( \hat a \hat b ) ( \hat a \hat b )$}\coordHE{}. 
...
summarizing over \myHighlight{$b, \nu$}\coordHE{} 
\begin{equation}\coord{}\boxEquation{
\begin{split}
\sum_{b, \nu} 
  \left (
...
Finally, after summarizing over \myHighlight{$a, \mu$}\coordHE{} we have 
\begin{equation}\coord{}\boxEquation{
\begin{split}
\sum_{a, \mu} \sum_{b, \nu} 
  \left (
...
between \myHighlight{$b,a$}\coordHE{} and \myHighlight{$\nu , \mu$}\coordHE{}.
\par
\noindent
If \myHighlight{$b = a$}\coordHE{} and \myHighlight{$\nu = \mu$}\coordHE{} 
\begin{equation}\coord{}\boxEquation{
\left (
...
\myHighlight{$(\hat A^a_\mu \hat A^{a\mu})(\hat A^b_\nu \hat A^{b \nu})$}\coordHE{}. 
If we average some quantum state \myHighlight{$| Q \rangle$}\coordHE{} we will have 
\begin{equation}\coord{}\boxEquation{
\begin{split}
  \left\langle Q | 
...
here \myHighlight{$(something)$}\coordHE{} is connected with commutators and permutations of 
brackets for three factors (for example, 
\myHighlight{$(\hat A^a_\mu \hat A^b_\nu) \hat A^{a \mu} \rightarrow 
\hat A^a_\mu (\hat A^b_\nu \hat A^{a \mu})$}\coordHE{} 
...
\myHighlight{$b, \nu$}\coordHE{} in Eq. \eqref{sec4-15} give us 
\begin{equation}\coord{}\boxEquation{
\begin{split}
  \sum_{b, \nu} 
  \left(
...
Now we can summarize over \myHighlight{$a, \mu$}\coordHE{} and Eq. \eqref{sec4-15} 
...
\myHighlight{$(6 \lambda A^a_\mu A^{a\mu})$}\coordHE{} 
...
in one point \myHighlight{$(x)$}\coordHE{}. 
...
state \myHighlight{$| Q \rangle$}\coordHE{} and the action of the field operator 
\myHighlight{$\hat A^a_\mu$}\coordHE{} on this state is 
\myHighlight{$|\Phi_1 \rangle = \hat A^a_\mu |Q \rangle$}\coordHE{}. For the nonlinear term the 
...
\myHighlight{$(\hat A^{a\mu} \hat A^{b\nu})$}\coordHE{} on the quantum state \myHighlight{$|Q \rangle$}\coordHE{}. 
...
where \myHighlight{$| \Phi_1 \rangle = \hat A^a_\mu | Q \rangle$}\coordHE{} and so on 
for every \myHighlight{$| \Phi_i \rangle$}\coordHE{}, \myHighlight{$i=2,3,4$}\coordHE{}. 
...
of such theory leads to the appearance of \myHighlight{$m^2 A^2$}\coordHE{}-like 
