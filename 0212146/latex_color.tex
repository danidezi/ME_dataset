...
\providecommand{\gi}{gauge-invariant}
\providecommand{\git}{gauge invariant}
...
where \myHighlight{$\theta^{ij}$}\coordHE{} is a constant, anti-symmetric two-index object, has arisen recently from
...
We consider the motion of a charged (\myHighlight{$e$}\coordHE{}) and massive (\myHighlight{$m$}\coordHE{}) particle in a constant magnetic
field (\myHighlight{$B$}\coordHE{}) pointing along the \myHighlight{$z$}\coordHE{}~direction.  All interesting physics is in the \myHighlight{$x$}\coordHE{}--\myHighlight{$y$}\coordHE{} plane. The
Lagrangian for this planar motion is
\numeq{n2}{
L = \fract12 m(\dot x^2 +\dot y^2) + \frac ec (\dot x A_x + \dot y A_y) - V(x,y) 
}
where the vector potential \myHighlight{$\vA$}\coordHE{} can be chosen as \myHighlight{$(0, xB)$}\coordHE{} and \myHighlight{$V(x,y)$}\coordHE{} describes additional
interactions (``impurities''). In the absence of \myHighlight{$V$}\coordHE{}, the quantum spectrum consists of the
well-known Landau levels \myHighlight{$\ket{N,d}$}\coordHE{}, where \myHighlight{$N$}\coordHE{} indexes the level's energy eigenvalue,
and~\myHighlight{$d$}\coordHE{} describes the infinite degeneracy of each level. The separation between levels is
\myHighlight{$O(B/m)$}\coordHE{}, so that in the strong magnetic field limit only the lowest Landau level \myHighlight{$\ket{0,d}$}\coordHE{} is
relevant. But observe that the large \myHighlight{$B$}\coordHE{} limit corresponds to small~\myHighlight{$m$}\coordHE{}, so projection on the
lowest Landau level is also achieved by setting \myHighlight{$m$}\coordHE{} to zero in \refeq{n2}. In that limit the
Lagrangian \refeq{n2}, in the chosen gauge, becomes
\numeq{n3}{
L_{\ell L\ell} = \frac ec Bx\dot y - V(x,y)\ . 
}
This is of the form \myHighlight{$p\dot q - H(p,q)$}\coordHE{}, and immediately identifies \myHighlight{$\frac ec Bx$}\coordHE{} and \myHighlight{$y$}\coordHE{} as
...
eigenvalues of \myHighlight{$V(x,y)$}\coordHE{}, where \myHighlight{$x$}\coordHE{} and \myHighlight{$y$}\coordHE{} are noncommuting.]
...
We evaluate \refeq{n6} by inserting intermediate states in product \myHighlight{$xy$}\coordHE{}:
\numeq{n7}{
...
vanishes: \myHighlight{$x$}\coordHE{} and \myHighlight{$y$}\coordHE{} do commute! But if one pretends that the world is restricted to the
...
The answer is that the (infinitesimal) transformation vector function \myHighlight{$f^i(x)$}\coordHE{} must be
determined by a scalar \myHighlight{$f(x)$}\coordHE{} through the expression~\cite{r3}
\numeq{e3}{
f^i (x) = \theta^{ij} \partial_j f(x) \ . 
}
Since \myHighlight{$\partial_i f^i(x) = 0$}\coordHE{},   these are recognized as volume-preserving transformations.
[They do not exhaust all volume preserving transformations, except in two dimensions. In
dimensions greater  two,
\refeq{e3} defines a subgroup of volume-preserving transforms that also leave \myHighlight{$\theta^{ij}$}\coordHE{}
invariant.]
...
elements are labeled, first by a discrete index~\myHighlight{$n$}\coordHE{}: \myHighlight{$\vec X_n(t)$}\coordHE{} is the position as a function of
time of the \myHighlight{$n$}\coordHE{}th fluid element.  Then one passes to a continuous labeling variable \myHighlight{$n \to \vec
...
\myHighlight{$-i$}\coordHE{}~times commutators, \`a la Dirac. More specifically, we shall see that the noncommuting field
...
(It is assumed that \myHighlight{$\theta^{ij}$}\coordHE{} has an inverse.)
Then \refeq{e4} is equivalent to the suggestive expression
\numeq{e8}{
\delta_f \hat A_i = \partial_i f + \bigl\{\hat A_i, f\bigr\}\ .
}
When the bracket is replaced by \myHighlight{$(-i)$}\coordHE{} times the commutator, this is precisely the gauge
...
given in terms of a Lie derivative \myHighlight{$L_f$}\coordHE{}:
\begin{gather}\coord{}\boxAlignEqnarray{
...
Thus, if the coordinate transformation  generated by \myHighlight{$f^\alpha$}\coordHE{} is supplemented by a  gauge
transformation generated by \myHighlight{$-f^\alpha A_\alpha$}\coordHE{}, the result is a gauge covariant coordinate
transformation
\numeq{e15}{
\delta'_f A_\mu = f^\alpha F_{\alpha\mu}
}
and the modified response of \myHighlight{$F_{\mu\nu}$}\coordHE{} involves the gauge-covariant Lie derivative \myHighlight{$L'_f$}\coordHE{}:
\numeq{e16}{
...
greater, because in general the coordinate transformation functions \myHighlight{$f^\alpha$}\coordHE{} do not
commute with the fields \myHighlight{$A_\mu, F_{\mu\nu}$}\coordHE{}; moreover, multiplication of \myHighlight{$x$}\coordHE{}-dependent
...
potential \myHighlight{$\hat A_\mu$}\coordHE{}, generated by \myHighlight{$f^\alpha (X)$}\coordHE{}, is 
\numeq{en25}{
\delta'_f \hat A_\mu = \fract12 \bigl\{
f^\alpha(X), \hat F_{\alpha\mu}\bigr\} + \mbox{reordering terms.}
}
Note that the generating function \myHighlight{$f^\alpha(X)$}\coordHE{} enters the anticommutator with covariant
argument~\myHighlight{$X$}\coordHE{}. \myHighlight{$f^\alpha$}\coordHE{} is restricted to be either linear or volume-preserving; in the latter
...
potential \myHighlight{$\hat A_\mu$}\coordHE{}  by a nonlocal function of a commuting potential \myHighlight{$a_\mu$}\coordHE{} and
...
commuting gauge potential \myHighlight{$a_\mu$}\coordHE{}; \myHighlight{$G(a,\lambda)$}\coordHE{} is the noncommuting gauge function that
transforms the noncommuting gauge potential \myHighlight{$\hat A_\mu$}\coordHE{}. \myHighlight{$G$}\coordHE{} depends on \myHighlight{$a_\mu$}\coordHE{}
...
invariant   [see \refeq{e8}]. Owing to the conservation of \myHighlight{$j^\mu$}\coordHE{}   [see \refeq{e20}], its dual 
\myHighlight{$\eps_{\alpha\beta\mu} j^\mu$}\coordHE{} satisfies a conventional, commuting Bianchi identity, and
therefore can be written as the curl of an Abelian vector potential~\myHighlight{$a_\alpha$}\coordHE{}, apart from
...
where the \myHighlight{$x$}\coordHE{} integral over an operator (\myHighlight{$\vec X$}\coordHE{}) dependent integrand is interpreted as a
