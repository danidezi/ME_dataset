...
The result of this deformation is just a change of the boundary condition of \myHighlight{$X$}\coordHE{} from Neumann to 
Dirichlet. 
...
The scalar \myHighlight{$\phi$}\coordHE{} is tachyonic with \myHighlight{$m_\phi^2=-2\phi_0^2$}\coordHE{}, and thus the vacuum \myHighlight{$\phi=0$}\coordHE{} is unstable. 
The stable vacua are \myHighlight{$\phi=\pm\phi_0$}\coordHE{}. 
The fluctuation \myHighlight{$\varphi$}\coordHE{} around the stable vacua is massive with \myHighlight{$m_\varphi^2=4\phi_0^2$}\coordHE{}. 
...
flux \cite{SUGRA}. 
The scalars \myHighlight{$\phi^I$}\coordHE{} may couple to the gauge fields \myHighlight{$A_\mu^a$}\coordHE{}. 
Thus the indices \myHighlight{$I,J$}\coordHE{} may include the gauge indices \myHighlight{$a,b$}\coordHE{}. 

General classical solutions of the action (\ref{gauged}) would contain nontrivial flux of the gauge fields and 
two-form field \myHighlight{$B_{\mu\nu}$}\coordHE{}. 
...
we simply set \myHighlight{$A^a_\mu,B_{\mu\nu}=0$}\coordHE{}. 
...
assume that all the scalars \myHighlight{$\phi^I$}\coordHE{} are constant: \myHighlight{$\phi^I=\phi_0^I$}\coordHE{}. 
Then the equations of motion of the scalars require that \myHighlight{$\phi_0^I$}\coordHE{} correspond to an extremum of 
the scalar potential \myHighlight{$V(\phi)$}\coordHE{}. 
...
A typical solution to the above equations with \myHighlight{$V(\phi_0)\ne0$}\coordHE{} is the linear dilaton background 
...
Note that a constant \myHighlight{$\Phi$}\coordHE{} is allowed to be a solution only when \myHighlight{$V(\phi_0)=0$}\coordHE{}. 
...
In particular, one can easily make a more strong statement when \myHighlight{$V(\phi_0)>0$}\coordHE{} and \myHighlight{$\Phi$}\coordHE{} is static. 
...
Denote the corresponding point as \myHighlight{$P$}\coordHE{}. 
At \myHighlight{$P$}\coordHE{}, the following equations are satisfied
\begin{equation}\coord{}\boxEquation{
\partial_k\Phi|_P=0, \hspace{5mm} \partial_k\partial_k\Phi|_P<0\ \ (\mbox{no summation for }k),
}{
\partial_k\Phi|_P=0, \hspace{5mm} \partial_k\partial_k\Phi|_P<0\ \ (\mbox{no summation for }k),
}{ecuacion}\coordE{}\end{equation}
where \myHighlight{$k$}\coordHE{} runs over spatial directions. 
...
theory is the space of all two-dimensional theories, denoted as \myHighlight{${\cal S}$}\coordHE{}, which can be defined only formally 
\cite{X^2}. 
...
we have to restrict ourselves to a subspace \myHighlight{${\cal S}_{weak}\subset{\cal S}$}\coordHE{} in which the 
...
Thus our interest is in another subspace \myHighlight{${\cal S}'\subset{\cal S}$}\coordHE{}, where
\begin{eqnarray}\coord{}\boxAlignEqnarray{\leftCoord{}
{\rightCoord{}\leftCoord{}\cal S}' &=& \{\mbox{space of all non-linear sigma models}\} \nonumber \rightCoord{}\\
&\leftCoord{}=& \{\mbox{space of all configurations of }G_{\mu\nu},\ B_{\mu\nu},\ \Phi,\cdots\}.\rightCoord{}
\rightCoord{}}{0mm}{3}{5}{
{\cal S}' &=& \{\mbox{space of all non-linear sigma models}\} \\
&=& \{\mbox{space of all configurations of }G_{\mu\nu},\ B_{\mu\nu},\ \Phi,\cdots\}.
}{1}\coordE{}\end{eqnarray}
In a subspace \myHighlight{${\cal S}'_{weak}={\cal S}'\cap{\cal S}_{weak}$}\coordHE{} in which the string coupling is small, the tree-level 
approximation of string theory is valid. 
For simplicity, we consider a more small subspace \myHighlight{${\cal S}'_{const}\subset{\cal S}'_{weak}$}\coordHE{} in which the 
...
In this case, the Virasoro algebra has a central charge \myHighlight{$c=\beta_\Phi$}\coordHE{}. 
...
Note that the scalar potential does not appear in \myHighlight{$\beta_{G_{\mu\nu}}$}\coordHE{}. 
...
Note that we have not defined the central charge on whole \myHighlight{${\cal S}'_{const}$}\coordHE{}, and thus we cannot prove the 
existence of a 
...
\myHighlight{$V(\phi^I_0)=0$}\coordHE{}. 
...
flow, but analyses outside \myHighlight{${\cal S}'_{weak}$}\coordHE{} would be no longer reliable. 
...
Suppose that the potential \myHighlight{$V(\phi)$}\coordHE{} does not depend on the dilaton \myHighlight{$\Phi$}\coordHE{} so that one can take an arbitrarily 
