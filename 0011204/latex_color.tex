...
We obtain the spectrum of glueball masses for the \myHighlight{${\cal N}=1$}\coordHE{} non-conformal 
...
dimensional cone with base a five dimensional Einstein manifold \myHighlight{$X_5$}\coordHE{}, one is lead 
to conjecture that type IIB string theory on \myHighlight{$AdS_5 \times X_5$}\coordHE{} is dual to the low 
energy limit of the worldvolume theory on the D3-branes at the singularity. In 
particular, a set of \myHighlight{$N$}\coordHE{} D3-branes at the conifold singularity results on a 
\myHighlight{${\cal N} =1$}\coordHE{} superconformal field theory with  \myHighlight{$SU(N) \times SU(N)$}\coordHE{} gauge group \cite{KW1}. 
...
the collapsed two-cycles of the singularity. In particular, the addition of \myHighlight{$M$}\coordHE{} fractional 
branes at the singular point of the conifold modifies the gauge group of the field theory to 
\myHighlight{$SU(N+M) \times SU(N)$}\coordHE{}. 
This theory was first investigated in \cite{KN} to leading order in \myHighlight{$M/N$}\coordHE{}. This solution was 
...
and the theory is \myHighlight{${\mathcal N}=1$}\coordHE{}  \myHighlight{$SU(M)$}\coordHE{} with no matter; this theory 
...
in the present letter find glueball masses
 for \myHighlight{${\cal N}=1$}\coordHE{}  \myHighlight{$SU(M)$}\coordHE{} Yang-Mills. In section 2 we will 
...
singularity dual to type IIB on a \myHighlight{$AdS_5 \times T^{11}$}\coordHE{} background. The corresponding 
gauge theory is 
\myHighlight{${\mathcal N} =1$}\coordHE{} supersymmetric with  \myHighlight{$SU(N) \times SU(N)$}\coordHE{} 
gauge group, and matter content \myHighlight{$A_{i}$}\coordHE{},\myHighlight{$B_{i}$}\coordHE{} where \myHighlight{$i=1,2$}\coordHE{}. The chiral fields 
transform as \myHighlight{$({\bf N}, 
{\bar {\bf N}})$}\coordHE{} and  \myHighlight{$({\bar {\bf N}},{\bf N})$}\coordHE{} respectively. 
The superpotential is \myHighlight{$W = \lambda \epsilon^{ij} \epsilon_{kl} \hbox {Tr } A_i B_k A_j B_l$}\coordHE{} . 
In the presence of M fractional branes the superpotential and matter
content are the same but the gauge group changes to \myHighlight{$SU(N+N)\times SU(N)$}\coordHE{}. The chiral
superfields are  now in the representation \myHighlight{$({\bf N+M}, {\bar {\bf N}})$}\coordHE{}. The supergravity 
equations corresponding to this situation were solved, to leading 
order in \myHighlight{$M/N$}\coordHE{}, in \cite{KN}, where the relative gauge coupling \myHighlight{$g_1^{-2}-g_2^{-2}$}\coordHE{} was 
...
collapsed \myHighlight{$S^2$}\coordHE{} of \myHighlight{$T^{11}$}\coordHE{}. D5-branes are sources for 
the magnetic R-R three-form flux through the \myHighlight{$S^3$}\coordHE{} cycle of \myHighlight{$T^{11}$}\coordHE{} and thus 
adding M fractional D3-branes implies that  the supergravity dual 
of this field theory will involve \myHighlight{$M$}\coordHE{} units of three-form flux,
\begin{equation}\coord{}\boxEquation{
\int_{S^3} F_3 = M,
\label{2}
}{
...
in addition to  \myHighlight{$N$}\coordHE{} units of five-form flux coming from the D3-branes,
\begin{equation}\coord{}\boxEquation{
\int_{T^{11}} F_5 = N.
\label{3}
}{
...
breaking. As a consequence of this flux, the two-form \myHighlight{$B_2$}\coordHE{} is no longer constant; it 
...
account the back reaction of \myHighlight{$H_3 = dB_2$}\coordHE{} and \myHighlight{$F_3$}\coordHE{} on other fields. This exact solution 
exhibits an effect which was hidden to leading order in \myHighlight{$M$}\coordHE{}: since 
\myHighlight{$F_5 = dC_4 +B_2 \wedge F_3$}\coordHE{}, \myHighlight{$F_5$}\coordHE{} also acquires a radial dependence,
\begin{equation}\coord{}\boxEquation{
...
with \myHighlight{$a$}\coordHE{} a constant of order unity. But there is a novelty arising from this 
solution: the five-form flux present at the  
ultraviolet scale \myHighlight{$r=r_0$}\coordHE{} may disappear once we reach the scale \myHighlight{$r=\tilde{r}$}\coordHE{}, where 
\myHighlight{${\cal K}(\tilde{r})=0$}\coordHE{}. This phenomenon can be related to the fact that the flux 
\myHighlight{$\int_{S^2} B_2$}\coordHE{} is not a periodic variable in the supergravity solution, because as 
this flux goes through a period, \myHighlight{${\cal K}(r) \rightarrow {\cal K}(r) -M$}\coordHE{}, so that 
the five-form flux is decreased by \myHighlight{$M$}\coordHE{} units. This decrease represents a renormalization 
...
a naked singularity at \myHighlight{$r=r_s$}\coordHE{}, which is the point where the harmonic function vanishes, \myHighlight{$h(r_s)=0$}\coordHE{}. 
From a physical point of view the singularity represents the end point of the cascade as 
the theory flows to the infrared, because negative values of \myHighlight{$N$}\coordHE{} are unphysical. And it is this 
...
with the singularity removed through the blow up of the \myHighlight{$S^3$}\coordHE{} in \myHighlight{$T^{11}$}\coordHE{}. A simple argument 
in favor of this suggestion comes from the origin of this singularity; it 
can be related to the divergent energy of the \myHighlight{$F_3$}\coordHE{} field. As there are \myHighlight{$M$}\coordHE{} units of 
flux of \myHighlight{$F_3$}\coordHE{} through the \myHighlight{$S^3$}\coordHE{} in \myHighlight{$T^{11}$}\coordHE{}, when \myHighlight{$S^3$}\coordHE{} shrinks to zero size \myHighlight{$F_3$}\coordHE{} 
diverges. However, if \myHighlight{$S^3$}\coordHE{} is kept of finite size, as in the deformed conifold, there is 
no need for \myHighlight{$F_3$}\coordHE{} to diverge.
...
the field theory. The authors of \cite{KS} showed how the \myHighlight{$U(1)$}\coordHE{} (\myHighlight{${\bf Z}_{2M}$}\coordHE{}, to be 
more precise) R-symmetry is broken to a \myHighlight{${\bf Z}_2$}\coordHE{} symmetry. This is indeed the symmetry 
left unbroken on the deformed conifold (\ref{7}), \myHighlight{$z_k \rightarrow - z_k$}\coordHE{}, instead of the 
original \myHighlight{$U(1)$}\coordHE{}  \myHighlight{$z_k \rightarrow e^{i \alpha} z_k$}\coordHE{}. The metric was then shown to be of 
the form 
\begin{equation}\coord{}\boxEquation{
...
with \myHighlight{$ds_6^2$}\coordHE{} the metric of the deformed conifold. In the basis \myHighlight{$\{\tau, g^{i=1,\ldots,5} 
(\psi,\theta_1,\theta_2,\phi_1,\phi_2) \}$}\coordHE{} of reference  
...
\myHighlight{$\alpha$}\coordHE{} is \myHighlight{$\alpha \sim (g_s M)^2$}\coordHE{}.
  
The solution contains a five-form and three-form flux.  \myHighlight{$F_5 = {\cal F}_5 + 
\star {\cal F}_5$}\coordHE{} is  given by;
\begin{equation}\coord{}\boxEquation{
...
gauge group factors by \myHighlight{$M$}\coordHE{} units, the D3-brane  goes to  zero and  
only the \myHighlight{$M$}\coordHE{} fractional D3-branes remain. Thus, at the bottom of the cascade we are left 
with a pure \myHighlight{${\cal N}=1$}\coordHE{} Yang-Mills theory 
with \myHighlight{$M$}\coordHE{} isolated vacua. In this letter we will use the above 
...
\myHighlight{$g_{\muh \nuh}$}\coordHE{}, a dilaton \myHighlight{$\Phi$}\coordHE{}, a zero-form R-R field C, 
two tensors --the NS-NS and R-R fields \myHighlight{$B_{\muh \nuh}$}\coordHE{} and 
\myHighlight{$C_{\muh \nuh}$}\coordHE{}-- and a R-R four-form \myHighlight{$C_{\sigmah \lambdah \tauh \nuh}$}\coordHE{}. In the present
...
the \myHighlight{$S^2$}\coordHE{} shrinks to zero but the \myHighlight{$S^3$}\coordHE{}
does not we can  expand in Kaluza-Klein 
modes over the \myHighlight{$S^3$}\coordHE{}. The expansion for the dilaton is, 
\begin{equation}\coord{}\boxEquation{
...
where \myHighlight{$x= x^\mu$}\coordHE{} are four dimensional worldvolume coordinates are
\myHighlight{$y=x^i$}\coordHE{} are coordinates over the three sphere. The Lorentz type gauges 
\myHighlight{$D^{m} A_{mn}=0$}\coordHE{}, \myHighlight{$D^{m} A_{m \mu} =0$}\coordHE{} can be used in order to fix 
\myHighlight{$a_{\mu}^{I_1}=a^{I_5}=0$}\coordHE{} in (\ref{aex}). 
...
the \myHighlight{$s$}\coordHE{}-wave of the kinetic side becomes \myHighlight{$D^{\mu} \partial_{\mu} \phi(x) \, Y(y)$}\coordHE{}, 
where we have omitted the \myHighlight{$k=0$}\coordHE{} index. However, the three-form of the Klebanov-Strassler 
solution \cite{KS} lives only on the internal coordinates so its spherical harmonic 
expansion involves \myHighlight{$Y_{[m n]}$}\coordHE{} but no scalar harmonics. Thus, 
the \myHighlight{$s$}\coordHE{}-wave equation for the dilaton becomes  
\myHighlight{$D^{\mu} \partial_{\mu} \phi (x)=0$}\coordHE{} or, explicitly,
\begin{equation}\coord{}\boxEquation{
...
branes we should consider the \myHighlight{$D^{\hat{\alpha}} G_{\hat{\alpha} \mu \nu}$}\coordHE{} part of equation 
(3.2). Using
\begin{equation}\coord{}\boxEquation{
...
together with  (\ref{dex}) and (\ref{aex})  the \myHighlight{$s$}\coordHE{}-wave equation of motion 
...
where \myHighlight{$[ \: \: ]$}\coordHE{} denotes antisymmetrization with strength one, and \myHighlight{$a_{\mu \nu}$}\coordHE{} is the 
...
field \myHighlight{$a_{\mu \nu}$}\coordHE{}. The \myHighlight{$SU(N+M) \times SU(N)$}\coordHE{} conifold theory has a \myHighlight{$U(1)_R$}\coordHE{} global 
symmetry \cite{KT}. In the present case, the deformation of the conifold breaks the 
\myHighlight{$U(1)_R$}\coordHE{} to \myHighlight{${\bf Z}_2$}\coordHE{}. Therefore, one expects a massless glueball in this theory. However, 
...
In \cite{GKP,Witten1} the correspondence between the \myHighlight{$AdS_5 
\times S^5$}\coordHE{} background and primary chiral fields correlators was made explicit. 
This correspondence should, in principle, be modified for the deformed conifold background 
since the space is not asymptotically \myHighlight{$AdS$}\coordHE{} anymore. However, as argued in \cite{KS}, 
only operators with \myHighlight{$\Delta < \frac {3}{2} M$}\coordHE{} can propagate all the way to 
\myHighlight{$\tau =0$}\coordHE{}. For this type of operators one expects that the correspondence should 
not be greatly modified. Thus, one expects that the dilaton will couple to the 
dimension four operator \myHighlight{$\hbox {Tr } 
F_{\mu \nu} F^{\mu \nu}$}\coordHE{}, so that the spin zero glueball masses, which can be derived 
...
\myHighlight{$g_s M \rightarrow 0$}\coordHE{} limit. In this limit the curvature of the space is
...
\myHighlight{$\phi (\tau,x) = f(\tau) e^{ik.x}$}\coordHE{}, so that 
a mode of momentum \myHighlight{$k$}\coordHE{} has a mass  \myHighlight{$m^2 = - k^2$}\coordHE{} and using the metric (\ref{8})
...
require that near the origin the function \myHighlight{$f$}\coordHE{} must be smooth, so that \myHighlight{$df/d \tau =0$}\coordHE{} at 
\myHighlight{$\tau = 0$}\coordHE{}. The asymptotics of \myHighlight{$f(\tau)$}\coordHE{} as \myHighlight{$\tau \rightarrow \infty$}\coordHE{} is obtained by 
...
\myHighlight{$\int \! \sqrt{g} \: \: |\Phi|^2$}\coordHE{} signals an exponential behaviour of the solution 
near infinity, \myHighlight{$f \sim e^{n \tau}$}\coordHE{}, as can be suspected from direct inspection 
of (\ref{b}). Changing  variables to \myHighlight{$f(\tau) = \psi(\tau) e^{n \tau}$}\coordHE{}, the wave 
...
This equation  can be solved for large values of \myHighlight{$\tau$}\coordHE{} by \myHighlight{$\psi(\tau) = c_1 e^{-n \tau} + 
c_2 e^{- \frac {1}{3} (3n +4) \tau}$}\coordHE{} which implies  
\myHighlight{$f(\tau) = c_1 + c_2 e^{-4/3 \tau}$}\coordHE{}. Normalizability requires \myHighlight{$c_1=0 $}\coordHE{} and we 
can fix \myHighlight{$c_2=1$}\coordHE{}. Thus, at infinity \myHighlight{$f \sim e^{-4/3 \tau}$}\coordHE{}. 
Equation (\ref{b}) becomes then an eigenvalue problem; we want to find the values of \myHighlight{$k$}\coordHE{} 
for which the equation has solutions matching the 
boundary conditions at \myHighlight{$0$}\coordHE{} and \myHighlight{$\infty$}\coordHE{}. We have done this numerically using 
a shooting technique \cite{gb1}: boundary conditions at infinity are taken 
as initial conditions and \myHighlight{$k$}\coordHE{} is adjusted  so that when numerically integrating 
(\ref{b}) \myHighlight{$f$}\coordHE{} is also smooth at \myHighlight{$\tau=0$}\coordHE{}. 
We should note that, numerically, infinity is taken as some large value of 
\myHighlight{$\tau$}\coordHE{} such that \myHighlight{$\tau \gg k^2$}\coordHE{}. On the other hand, since the boundary 
condition 
requires that \myHighlight{$f$}\coordHE{} goes exponentially fast to zero at infinity, there is a 
...
The glueball masses 
thus obtained are shown in Table~1. They are measured in units of \myHighlight{$\epsilon^{4/3}$}\coordHE{} which, 
...
Next we will study the equation of motion for the complexified two-form \myHighlight{$a_{\mu \nu}$}\coordHE{}. 
...
where we have impossed solutions to be of the form \myHighlight{$a_{\mu \nu} (\tau,x) = 
f_{\mu \nu}(\tau) e^{ik.x}$}\coordHE{}, and summation is implied over the \myHighlight{$\alpha$}\coordHE{} and 
...
\myHighlight{$f_{\mu \nu} (\tau) = b_{\mu \nu} (\tau) + i c_{\mu \nu} (\tau)$}\coordHE{}. Impossing the gauge fixing 
condition \myHighlight{$a_{\mu \tau} =0$}\coordHE{}, so that the two-form has no components along the 
...
Unlike the \myHighlight{$AdS_5 \times S^5$}\coordHE{} case, the Klebanov-Strassler background
...
a Kaluza-Klein decomposition on this \myHighlight{$S^3$}\coordHE{}. We 
...
breaking of the \myHighlight{$U(1)_R$}\coordHE{} symmetry.
...
\bibitem{krasnitz}
M. Krasnitz, ``A Two Point Function in a Cascading \myHighlight{${\cal N} =1$}\coordHE{} Gauge 
