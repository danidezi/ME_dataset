...
\providecommand\PF[4]{{{}^{#1}\Psi^{#2}_{#3,#4}}}
...
\providecommand\mat{{\mbox{Mat}_N}}
\providecommand\AXB{{{}^{\cal A}\!X^{\cal B}}}
...
\myHighlight{$\Psi=\Psi^{matter}\otimes\Psi^{ghost}$}\coordHE{}
...
where \myHighlight{$U_n$}\coordHE{} describes the conformal transformations 
\cite{r-RZ, r-RSZ4},
\begin{equation}\coord{}\boxEquation{
...
\myHighlight{$|\mbox{vac}; a, a\rangle$}\coordHE{} is the vacuum state
in the open string sector with the both ends attached
to the same D-brane labeled as \myHighlight{$a$}\coordHE{}. It was shown in \cite{r-RZ}
that they satisfy
\begin{equation}\coord{}\boxEquation{
...
Clearly \myHighlight{$n=1$}\coordHE{} (the identity) and
\myHighlight{$n=\infty$}\coordHE{} (the sliver) give the projection operators.
...
\begin{enumerate}
 \item It defines the projection to the open string states with
       ...
       \myHighlight{$\cI^a=|1\rangle_a$}\coordHE{} picks up the open string field algebra
       \myHighlight{$\cA^a$}\coordHE{} with the particular boundary condition \myHighlight{$a$}\coordHE{}.  Namely
\begin{equation}\coord{}\boxEquation{\label{e-identity_projector}
 \cI^a\star \cC \star\cI^a = \cA^a\,\,.
}{\cI^a\star \cC \star\cI^a = \cA^a\,\,.
}{ecuacion}\coordE{}\end{equation}
 \item While the trace of \myHighlight{$\cI^a$}\coordHE{} is apparently infinite, there is
...
Here \myHighlight{$\langle \mbox{vac}|$}\coordHE{} is the closed string vacuum
and \myHighlight{$|a\rangle$}\coordHE{} is the Cardy state.  The quantity 
...
expanded around a particular projector \myHighlight{$p$}\coordHE{}
has the \myHighlight{$n\times n$}\coordHE{} (\myHighlight{$n$}\coordHE{} is the rank of the projector \myHighlight{$p$}\coordHE{}) 
...
in terms of \myHighlight{$L_0$}\coordHE{} (Virasoro generator), there is a contribution
...
Suppose we start from \myHighlight{$N$}\coordHE{} D-branes with the boundary condition
specified by the Cardy  state \myHighlight{$|a\rangle$}\coordHE{}  \myHighlight{$a=1,\cdots,N$}\coordHE{}\footnote{
...
a \myHighlight{$N\times N$}\coordHE{} matrix \myHighlight{$\Psi$}\coordHE{} whose \myHighlight{$ab$}\coordHE{}-th component is
written as \myHighlight{$\Psi^{ab}$}\coordHE{}. \myHighlight{$\Psi^{ab}$}\coordHE{} describes the 
open string which connects D-branes  with labels \myHighlight{$a$}\coordHE{} and \myHighlight{$b$}\coordHE{}.
We have to define the (perturbative or conventional)
BRST operator \myHighlight{$Q$}\coordHE{} for each sector by combining the Virasoro operator
of the matter sector (which is different 
for each sector) and the ghost sector which is universal.
We will use \myHighlight{$Q_{ab}$}\coordHE{} if we want to specify the operator
on a particular sector. We write the matrix \myHighlight{$Q\Psi$}\coordHE{} 
to mean the matrix whose \myHighlight{$ab$}\coordHE{}-th component is
\myHighlight{$Q_{ab}(\Psi^{ab})$}\coordHE{}.
...
Here the integration \myHighlight{$\int_a$}\coordHE{}
is defined by the identity operator for the open string
in \myHighlight{$aa$}\coordHE{} channel. The summation with respect to 
\myHighlight{$a$}\coordHE{} gives the trace over Chan-Paton factor
...
where \myHighlight{$\epsilon_{ab}$}\coordHE{} is a string field from \myHighlight{$ab$}\coordHE{}-sector.
Appearance of non-abelian gauge symmetry even for 
the intertwining strings might sound strange.  
However in these cases
\myHighlight{$\epsilon_{ab}$}\coordHE{} gives only the massive gauge
symmetry and does not appear in the massless sector.

In this language, the description of the identity projector
becomes very simple.  Namely for each \myHighlight{$aa$}\coordHE{}-sector, one may define
the identity operator \myHighlight{$|1\rangle_a\otimes \cI^{gh}$}\coordHE{}
(\myHighlight{$\cI^{gh}$}\coordHE{} is the universal identity operator of the ghost field).
The projector \myHighlight{$\cI^a$}\coordHE{} is then
defined as putting it at \myHighlight{$aa$}\coordHE{}-th component and putting zero
...
We start from the discussion of \myHighlight{$N$}\coordHE{} D-25 branes
in the flat background.
Suppose that the open string field
\myHighlight{$\Psi^{aa}$}\coordHE{} whose both ends are connected to the \myHighlight{$a$}\coordHE{}-th
(\myHighlight{$1\leq a \leq N$}\coordHE{})
D-brane is described by the embedding function 
\myHighlight{$x_{aa}^\mu(\sigma)$}\coordHE{} (\myHighlight{$0\leq \sigma \leq \pi$}\coordHE{}, \myHighlight{$\mu=0,\dots,25$}\coordHE{})
with the Neumann boundary condition at \myHighlight{$\sigma=0,\pi$}\coordHE{}.
...
\myHighlight{$\ell(\sigma)$}\coordHE{} and \myHighlight{$r(\sigma)$}\coordHE{} thus have the boundary condition
...
At the midpoint, since we fix \myHighlight{$\ell(\frac{\pi}{2})=r(\frac{\pi}{2})=0$}\coordHE{},
we need an extra degree of freedom (the zero mode) and we write it
as \myHighlight{$\bar{x}$}\coordHE{} (note that it does not have the index of the boundary).

To describe the open string that intertwine different D-branes
(say \myHighlight{$a$}\coordHE{} and \myHighlight{$b$}\coordHE{}), we define the embedding function
by combining \myHighlight{$\ell_a$}\coordHE{} and \myHighlight{$r_b$}\coordHE{},
...
Thanks to the Dirichlet boundary condition at \myHighlight{$\sigma=\pi/2$}\coordHE{},
such a mixed combination becomes consistent.
The string field for \myHighlight{$a$}\coordHE{}-\myHighlight{$b$}\coordHE{} sector is defined as a
matrix indexed by \myHighlight{$\ell_a$}\coordHE{} and \myHighlight{$r_b$}\coordHE{}
\begin{equation}\coord{}\boxEquation{
...
\myHighlight{$e^{ip\bar x}$}\coordHE{} if the momentum of the open string is \myHighlight{$p$}\coordHE{}.

Up to this point, it is (at least intuitively) clear 
that our argument does not depend on the fact that we start from the 
\myHighlight{$N$}\coordHE{} identical D-25 branes.  Indeed the definition of
...
degree of freedom of the open string \myHighlight{$x^{ab}$}\coordHE{} belongs to the
twisted sector. We have to split it into two pieces and the question
is whether one may limit the boundary condition at \myHighlight{$\sigma=\pi/2$}\coordHE{} 
as Dirichlet.  In the generic BCFT, one may further need some
sort of discrete summation over sectors even at \myHighlight{$\sigma=\pi/2$}\coordHE{}. 
...
is made only for the \myHighlight{$N$}\coordHE{} identical D-branes while we will use
the universal notation.

In the split string formalism, the definition
of the identity projector becomes quite
intuitive.  We introduce the \myHighlight{$N\times N$}\coordHE{} 
matrix valued string field \myHighlight{$p$}\coordHE{}. It is defined that we put
\begin{equation}\coord{}\boxEquation{
\cI_{gh}\otimes \pi_{ab}\prod_\sigma\delta(\ell_a(\sigma)-r_b(\sigma))
}{
\cI_{gh}\otimes \pi_{ab}\prod_\sigma\delta(\ell_a(\sigma)-r_b(\sigma))
}{ecuacion}\coordE{}\end{equation}
at \myHighlight{$a$}\coordHE{}-\myHighlight{$b$}\coordHE{} th entry where \myHighlight{$\pi_{ab}$}\coordHE{} is a constant complex number
and \myHighlight{$\cI_{gh}$}\coordHE{} is the identity operator of the ghost sector
\cite{r-AAB, r-GT2}.  By the matrix multiplication, it satisfies
\myHighlight{$p^2=p$}\coordHE{} if \myHighlight{$\pi_{ab}$}\coordHE{} satisfies \myHighlight{$\sum_b\pi_{ab}\pi_{bc}=\pi_{ac}$}\coordHE{}.
We note that in the definition of the identity
operator, we have to equate the boundary conditions
in order that the delta function functional \myHighlight{$\delta(\ell_a-r_b)$}\coordHE{}
is consistent.

We comment that in the tachyon condensation process from D-\myHighlight{$(p+2)$}\coordHE{} brane
to D-\myHighlight{$p$}\coordHE{} branes \cite{r-HKLM, r-Witten2} 
in the Seiberg-Witten limit \cite{r-SW}, we have already met
the matrix \myHighlight{$\pi_{ab}$}\coordHE{}.  Indeed in this example one may take
\myHighlight{$p+1$}\coordHE{} directions to be Neumann for \myHighlight{$\ell_a^\mu$}\coordHE{} and \myHighlight{$r_a\mu$}\coordHE{}
and the zero mode algebra gives the matrix \myHighlight{$\pi_{ab}$}\coordHE{}.
It is the situation where the string field algebra
becomes the direct product of \myHighlight{$\cal B(H)\otimes\cA$}\coordHE{}
where \myHighlight{$\cA$}\coordHE{} is the open string algebra between D-\myHighlight{$p$}\coordHE{} branes.
The matrix \myHighlight{$\pi_{ab}$}\coordHE{} can be precisely 
...
of \myHighlight{$p-p'$}\coordHE{} system in \cite{r-CIMM}.
...
solution to \myHighlight{$\Psi^2=0$}\coordHE{},
...
(matter+ghost) and \myHighlight{$Q_L$}\coordHE{} is the perturbative BRST current
integrated over the left half of the open string.
By putting \myHighlight{$\Psi=\Psi_0+\psi$}\coordHE{} into 
(\ref{e-cubic}) gives,
\begin{equation}\coord{}\boxEquation{
...
projector \myHighlight{$\cI^a$}\coordHE{} and write it as
\myHighlight{$p$}\coordHE{} and the sum of the all the rest as \myHighlight{$q$}\coordHE{}. It is then 
...
gives also a solution to \myHighlight{$\Psi_0^2=0$}\coordHE{}.  
Here we assume that we can 
split the BRST charge for \myHighlight{$ab$}\coordHE{}-sector as,
\begin{equation}\coord{}\boxEquation{
 Q_{ab}=Q_{aL}+Q_{bR}\,\,.
}{
 Q_{ab}=Q_{aL}+Q_{bR}\,\,.
}{ecuacion}\coordE{}\end{equation}
Roughly speaking, \myHighlight{$Q_{aL}$}\coordHE{} (resp. \myHighlight{$Q_{bR}$}\coordHE{}) is the BRST charge
for the half strings \myHighlight{$\ell^a(\sigma)$}\coordHE{} (resp. \myHighlight{$r^b(\sigma)$}\coordHE{}).
...
If we note that \myHighlight{$Q_{L,R}$}\coordHE{} does not change the boundary conditions,
\myHighlight{$p\star Q_L^2 \ p= Q_L^2 \, p$}\coordHE{} and 
\myHighlight{$(Q_R Q_L\,  p)\star p=Q_R Q_L\,  p$}\coordHE{}.  Summing up these two formulae gives
\begin{equation}\coord{}\boxEquation{
...
\myHighlight{$Q_L p$}\coordHE{} {\em does not} give BRST operator for the
...
If we restrict  the string field to \myHighlight{$aa$}\coordHE{}-sector,
...
some undesirable terms.  If we put \myHighlight{$\Psi=Q_L\, p+\psi$}\coordHE{}
into the action,
...
Here \myHighlight{$\psi_0,t,\bar t$}\coordHE{} are components of \myHighlight{$\psi$}\coordHE{},
\begin{equation}\coord{}\boxEquation{
...
the D-branes picked up by the projector \myHighlight{$p$}\coordHE{} survives
after tachyon condensation.
The string field \myHighlight{$\psi_0$}\coordHE{} is then interpreted as the open strings
on the survived branes.  On the other hand,
two string fields \myHighlight{$t,\bar t$}\coordHE{} describe the intertwiner
between the survived and the disappeared ones.
Whereas we have a correct kinetic term  (\ref{e-kinetic})
for \myHighlight{$a$}\coordHE{}, it is not reasonable that we have
a sort of (``incomplete'') kinetic terms for \myHighlight{$t,\bar t$}\coordHE{}.
...
\myHighlight{$t,\bar t$}\coordHE{} as a kind of W-bosons for the symmetry breaking.
In the original action (\ref{e-cubic}), the gauge symmetry 
(gauge group on the configuration space of the open strings)
is generically \myHighlight{$U(\infty)$}\coordHE{} if we consider all possible
...
the form \myHighlight{$Q\epsilon$}\coordHE{} only when \myHighlight{$\epsilon$}\coordHE{} belongs to \myHighlight{$aa$}\coordHE{}-sector.
If it belongs to the \myHighlight{$oo'$}\coordHE{}-sector
with \myHighlight{$o\neq a, o'\neq a$}\coordHE{} then this term simply vanishes.
On the other hand, if it belongs to \myHighlight{$ao$}\coordHE{} or \myHighlight{$oa$}\coordHE{} with \myHighlight{$o\neq a$}\coordHE{}
...
the gauge symmetry \myHighlight{$U(\infty)$}\coordHE{} is broken to \myHighlight{$U(1)\oplus U(\infty-1)$}\coordHE{}.

We may guess the off diagonal gauge transformation may
remove the unwanted degree of freedom. Indeed, when \myHighlight{$t,\bar t$}\coordHE{}
...
We note that \myHighlight{$Q_{L,R}^2$}\coordHE{} is written  in terms of ghost 
...
of the action \myHighlight{$S(\Psi_0)$}\coordHE{} for any classical solution \myHighlight{$\Psi_0$}\coordHE{}
...
gauging away \myHighlight{$t,\bar t$}\coordHE{}, we will have a singular transformation
...
``Morita Equivalence and Continuous-Trace \myHighlight{$C^*$}\coordHE{}-algebra'',
