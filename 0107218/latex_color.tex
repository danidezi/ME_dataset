...
\providecommand{\gab}{G_{a b}}
\providecommand{\ftab}{\Tilde{\mathfrak F}_{a b}}
\providecommand{\gtab}{\Tilde{G}_{a b}}
\providecommand{\gt}{\Tilde{G}}
\providecommand{\ft}{\Tilde{\mathfrak F}}
\providecommand{\at}{\Tilde{A}}
\providecommand{\omrz}{1 - \dot{r}^2 Z}
\providecommand{\fL}{\mathcal{L}}
\providecommand{\fl}{\Sigma}
\providecommand{\rd}{\dot{r}}
\providecommand{\rdd}{\ddot{r}}
\providecommand{\rds}{\dot{r}^2}
\providecommand{\rdf}{\dot{r}^4}
\providecommand{\td}{\dot{t}}
\providecommand{\tdd}{\ddot{t}}
...
solution for an extremal black \myHighlight{$p$}\coordHE{}-brane with Q units of R-R charge
(see section 2).
These solutions (\myHighlight{$p \ne 3$}\coordHE{}) are singular.  However, as curvatures
...
Dq-brane can be resolved.  Namely, for \myHighlight{$\mbox{p} \ne 3$}\coordHE{} the proper
picture at short distances, where the induced metric is becoming singular,
is that of the gauge theory living on the brane.  For \myHighlight{$\mbox{p}=3$}\coordHE{} the
...
while \myHighlight{$m, n$}\coordHE{} run transverse to the brane.  Throughout this paper
we will use the gauge, 
\myHighlight{$\xi^0 = \tau \ $}\coordHE{}  \myHighlight{$\ \xi^i = X^i$}\coordHE{}.
Here \myHighlight{$\xi^a$}\coordHE{} are the coordinates on the brane.  Since the branes are
parallel the indices \myHighlight{$a,b$}\coordHE{} are a subset of the \myHighlight{$\mu, \nu$}\coordHE{}.  \myHighlight{$\tau$}\coordHE{} will
...
space) and lie along \myHighlight{$X^{p+1}, \ (r \equiv |X^{p+1}|)$}\coordHE{}.  Furthermore,
it will be assumed that \myHighlight{$Q \gg 1$}\coordHE{} so that the back reaction of the test
...
Here \myHighlight{$a(\tau) = Z^{-1/4}[r(\tau)]$}\coordHE{}.
\myHighlight{$\gtab$}\coordHE{}, will be interpreted as the cosmology on
the \myHighlight{$q+1$}\coordHE{} dimensional universe.
  
At this point we shall assume the naive perspective of a \myHighlight{$q+1$}\coordHE{}
...
For parallel branes if \myHighlight{$p \ne q$}\coordHE{} then \myHighlight{$\Tilde{C}_{q+1}$}\coordHE{}, the pullback
of the Ramond-Ramond potential, vanishes.  However, for 
\myHighlight{$p=q$}\coordHE{} \cite{johnsn}\footnote{In \cite{johnsn} \myHighlight{$\Tilde{C}_{q+1}$}\coordHE{} is
given in static gauge.  Here we have changed coordinates 
\myHighlight{$\xi^0 = X^0 \rightarrow \xi^0 = \tau$}\coordHE{}.},
$$\coord{}\boxMath{ 
...
\myHighlight{$s \equiv -(p-q-4)/4$}\coordHE{} Lagrangian takes the form, 
$$\coord{}\boxMath{
\fL  = Z^{-s} ( \sqrt{ \tds - Z \rds } - \td \delta_{s,1})+\td \delta_{s,1} \ .
}{dollar}{0pt}\coordE{}$$
Since \myHighlight{$t(\tau)$}\coordHE{} is a cyclic variable we know that 
\myHighlight{$\partial \fL / \partial \td = c + \delta_{s,1}$}\coordHE{}, 
where \myHighlight{$c$}\coordHE{} is a constant of the motion\footnote{Here we have added
\myHighlight{$\delta_{s,1}$}\coordHE{} to \myHighlight{$c$}\coordHE{} in order to simplify (\ref{tdot}).}.  Taking
...
where we have defined \myHighlight{$l_{p-q}^2 \equiv (Z^{s} c + \delta_{s,1})^2 - 1$}\coordHE{}.
The gauge choice (\ref{goo}) relates \myHighlight{$r(\tau) \ \mbox{and} \ t(\tau)$}\coordHE{}.
Therefore the equation of motion coming from the variation of \myHighlight{$\fL$}\coordHE{}
with respect to
\myHighlight{$r(\tau)$}\coordHE{} doesn't provide any new information.  From (\ref{goo})
...
over \myHighlight{$r$}\coordHE{}, however, gives rise to quartic roots and hypergeometric
functions which cannot be algebraically inverted to give \myHighlight{$r(\tau)$}\coordHE{}.
Fortunately the interesting behavior, namely the big bang/big crunch
takes place when the branes are nearly
coincident.  In this limit, \myHighlight{$r \ll \rho$}\coordHE{}, we can see that,
\begin{eqnarray*}\coord{}\boxAlignEqnarray{\leftCoord{}
...
Here we have assumed \myHighlight{$c \ge 0$}\coordHE{}, which is required for the induced metric
to be Lorentzian.
The induced metric on the brane is then,
$$\coord{}\boxMath{
ds^2 = - d\tau^2 + \mathcal{C}^2 (\frac{\tau}{\rho})^{2/3} dX^i dX^i \ .
}{dollar}{0pt}\coordE{}$$
A scale factor \myHighlight{$a(\tau) \propto \tau^{1/3}$}\coordHE{} corresponds to a perfect
...
There might be concern that the limit \myHighlight{$r \ll \rho$}\coordHE{} is
...
Note that for large \myHighlight{$(g Q)$}\coordHE{}, which is required since the
...
the induced scalar curvature on the brane, \myHighlight{$R$}\coordHE{}, is much less than
the string scale,  
\myHighlight{$R \ll 1 / \alpha^{\prime}$}\coordHE{}.  For our test brane universe 
\myHighlight{$R \propto \tau^{-2}$}\coordHE{}.
This can be seen by direct computation or by dimensional
arguments.  From this we can see \myHighlight{$\alpha^{\prime} \ll \tau^2$}\coordHE{}, 
which provides a lower bound on \myHighlight{$r$}\coordHE{}.  It follows that,
\begin{equation}\coord{}\boxEquation{
\mathcal{C}^2 (g Q)^{1/6} 
...
The upper bound comes from working in the region, \myHighlight{$r \ll \rho$}\coordHE{}.
Note that the lower bound can be taken to zero by choosing
\myHighlight{$\mathcal{C} \sim 0$}\coordHE{}.  This limit corresponds to having the test brane
...
the number of \myHighlight{$ND$}\coordHE{} coordinates\footnote{the number of coordinates such that a
...
caused by back-reaction on the metric we have taken the \myHighlight{$Q$}\coordHE{}, the number
